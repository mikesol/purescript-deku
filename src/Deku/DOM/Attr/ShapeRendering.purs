...
instance Attr Circle_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr ClipPath_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr ClipPath_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr Defs_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr Defs_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr Ellipse_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr Ellipse_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr FeBlend_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr FeBlend_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr FeColorMatrix_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr FeColorMatrix_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr FeComponentTransfer_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr FeComponentTransfer_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr FeComposite_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr FeComposite_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr FeConvolveMatrix_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr FeConvolveMatrix_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr FeDiffuseLighting_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr FeDiffuseLighting_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr FeDisplacementMap_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr FeDisplacementMap_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr FeFlood_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr FeFlood_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr FeGaussianBlur_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr FeGaussianBlur_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr FeImage_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr FeImage_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr FeMerge_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr FeMerge_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr FeMorphology_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr FeMorphology_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr FeOffset_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr FeOffset_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr FeSpecularLighting_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr FeSpecularLighting_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr FeTile_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr FeTile_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr FeTurbulence_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr FeTurbulence_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr Filter_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr Filter_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr ForeignObject_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr ForeignObject_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr G_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr G_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr Image_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr Image_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr Line_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr Line_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr LinearGradient_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr LinearGradient_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr Marker_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr Marker_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr Mask_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr Mask_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr Path_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr Path_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr Pattern_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr Pattern_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr Polygon_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr Polygon_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr Polyline_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr Polyline_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr RadialGradient_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr RadialGradient_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr Rect_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr Rect_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr Svg_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr Svg_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr Switch_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr Switch_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr Symbol_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr Symbol_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr Text_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr Text_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr TextPath_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr TextPath_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr Tspan_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr Tspan_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr Use_ ShapeRendering  String  where
  attr ShapeRendering value = unsafeAttribute (  
    { key: "shape-rendering", value: prop' value  } <$ _)
instance Attr Use_ ShapeRendering (Event.Event Unit -> Event.Event  String ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \value -> { key: "shape-rendering", value: prop' value })) eventValue)
...
instance Attr everything ShapeRendering (Event.Event Unit -> Event.Event  Unit ) where
  attr ShapeRendering eventValue = unsafeAttribute (map (map (
    \_ -> { key: "shape-rendering", value: unset' })) eventValue)
