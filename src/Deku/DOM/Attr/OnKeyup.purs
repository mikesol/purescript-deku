...
instance Attr anything OnKeyup (Zora Unit) where
  attr OnKeyup value = unsafeAttribute { key: "keyup", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnKeyup (Zora Boolean) where
  attr OnKeyup value = unsafeAttribute { key: "keyup", value: cb' (Cb (const (runImpure value))) }