...
instance Attr FeComposite_ K1 (Event.Event Unit -> Event.Event  String ) where
  attr K1 eventValue = unsafeAttribute (map (map ( \value ->
    { key: "k1", value: prop' value })) eventValue)
...
instance Attr everything K1 (Event.Event Unit -> Event.Event  Unit ) where
  attr K1 eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "k1", value: unset' })) eventValue)
