...
instance Attr FeBlend_ UnicodeBidi (Event.Event Unit -> Event.Event  String ) where
  attr UnicodeBidi eventValue = unsafeAttribute (map (map (
    \value -> { key: "unicode-bidi", value: prop' value })) eventValue)
...
instance Attr FeColorMatrix_ UnicodeBidi  String  where
  attr UnicodeBidi value = unsafeAttribute (  
    { key: "unicode-bidi", value: prop' value  } <$ _)
instance Attr FeColorMatrix_ UnicodeBidi (Event.Event Unit -> Event.Event  String ) where
  attr UnicodeBidi eventValue = unsafeAttribute (map (map (
    \value -> { key: "unicode-bidi", value: prop' value })) eventValue)
...
instance Attr FeComponentTransfer_ UnicodeBidi  String  where
  attr UnicodeBidi value = unsafeAttribute (  
    { key: "unicode-bidi", value: prop' value  } <$ _)
instance Attr FeComponentTransfer_ UnicodeBidi (Event.Event Unit -> Event.Event  String ) where
  attr UnicodeBidi eventValue = unsafeAttribute (map (map (
    \value -> { key: "unicode-bidi", value: prop' value })) eventValue)
...
instance Attr FeComposite_ UnicodeBidi  String  where
  attr UnicodeBidi value = unsafeAttribute (  
    { key: "unicode-bidi", value: prop' value  } <$ _)
instance Attr FeComposite_ UnicodeBidi (Event.Event Unit -> Event.Event  String ) where
  attr UnicodeBidi eventValue = unsafeAttribute (map (map (
    \value -> { key: "unicode-bidi", value: prop' value })) eventValue)
...
instance Attr FeConvolveMatrix_ UnicodeBidi  String  where
  attr UnicodeBidi value = unsafeAttribute (  
    { key: "unicode-bidi", value: prop' value  } <$ _)
instance Attr FeConvolveMatrix_ UnicodeBidi (Event.Event Unit -> Event.Event  String ) where
  attr UnicodeBidi eventValue = unsafeAttribute (map (map (
    \value -> { key: "unicode-bidi", value: prop' value })) eventValue)
...
instance Attr FeDiffuseLighting_ UnicodeBidi  String  where
  attr UnicodeBidi value = unsafeAttribute (  
    { key: "unicode-bidi", value: prop' value  } <$ _)
instance Attr FeDiffuseLighting_ UnicodeBidi (Event.Event Unit -> Event.Event  String ) where
  attr UnicodeBidi eventValue = unsafeAttribute (map (map (
    \value -> { key: "unicode-bidi", value: prop' value })) eventValue)
...
instance Attr FeDisplacementMap_ UnicodeBidi  String  where
  attr UnicodeBidi value = unsafeAttribute (  
    { key: "unicode-bidi", value: prop' value  } <$ _)
instance Attr FeDisplacementMap_ UnicodeBidi (Event.Event Unit -> Event.Event  String ) where
  attr UnicodeBidi eventValue = unsafeAttribute (map (map (
    \value -> { key: "unicode-bidi", value: prop' value })) eventValue)
...
instance Attr FeFlood_ UnicodeBidi  String  where
  attr UnicodeBidi value = unsafeAttribute (  
    { key: "unicode-bidi", value: prop' value  } <$ _)
instance Attr FeFlood_ UnicodeBidi (Event.Event Unit -> Event.Event  String ) where
  attr UnicodeBidi eventValue = unsafeAttribute (map (map (
    \value -> { key: "unicode-bidi", value: prop' value })) eventValue)
...
instance Attr FeGaussianBlur_ UnicodeBidi  String  where
  attr UnicodeBidi value = unsafeAttribute (  
    { key: "unicode-bidi", value: prop' value  } <$ _)
instance Attr FeGaussianBlur_ UnicodeBidi (Event.Event Unit -> Event.Event  String ) where
  attr UnicodeBidi eventValue = unsafeAttribute (map (map (
    \value -> { key: "unicode-bidi", value: prop' value })) eventValue)
...
instance Attr FeImage_ UnicodeBidi  String  where
  attr UnicodeBidi value = unsafeAttribute (  
    { key: "unicode-bidi", value: prop' value  } <$ _)
instance Attr FeImage_ UnicodeBidi (Event.Event Unit -> Event.Event  String ) where
  attr UnicodeBidi eventValue = unsafeAttribute (map (map (
    \value -> { key: "unicode-bidi", value: prop' value })) eventValue)
...
instance Attr FeMerge_ UnicodeBidi  String  where
  attr UnicodeBidi value = unsafeAttribute (  
    { key: "unicode-bidi", value: prop' value  } <$ _)
instance Attr FeMerge_ UnicodeBidi (Event.Event Unit -> Event.Event  String ) where
  attr UnicodeBidi eventValue = unsafeAttribute (map (map (
    \value -> { key: "unicode-bidi", value: prop' value })) eventValue)
...
instance Attr FeMorphology_ UnicodeBidi  String  where
  attr UnicodeBidi value = unsafeAttribute (  
    { key: "unicode-bidi", value: prop' value  } <$ _)
instance Attr FeMorphology_ UnicodeBidi (Event.Event Unit -> Event.Event  String ) where
  attr UnicodeBidi eventValue = unsafeAttribute (map (map (
    \value -> { key: "unicode-bidi", value: prop' value })) eventValue)
...
instance Attr FeOffset_ UnicodeBidi  String  where
  attr UnicodeBidi value = unsafeAttribute (  
    { key: "unicode-bidi", value: prop' value  } <$ _)
instance Attr FeOffset_ UnicodeBidi (Event.Event Unit -> Event.Event  String ) where
  attr UnicodeBidi eventValue = unsafeAttribute (map (map (
    \value -> { key: "unicode-bidi", value: prop' value })) eventValue)
...
instance Attr FeSpecularLighting_ UnicodeBidi  String  where
  attr UnicodeBidi value = unsafeAttribute (  
    { key: "unicode-bidi", value: prop' value  } <$ _)
instance Attr FeSpecularLighting_ UnicodeBidi (Event.Event Unit -> Event.Event  String ) where
  attr UnicodeBidi eventValue = unsafeAttribute (map (map (
    \value -> { key: "unicode-bidi", value: prop' value })) eventValue)
...
instance Attr FeTile_ UnicodeBidi  String  where
  attr UnicodeBidi value = unsafeAttribute (  
    { key: "unicode-bidi", value: prop' value  } <$ _)
instance Attr FeTile_ UnicodeBidi (Event.Event Unit -> Event.Event  String ) where
  attr UnicodeBidi eventValue = unsafeAttribute (map (map (
    \value -> { key: "unicode-bidi", value: prop' value })) eventValue)
...
instance Attr FeTurbulence_ UnicodeBidi  String  where
  attr UnicodeBidi value = unsafeAttribute (  
    { key: "unicode-bidi", value: prop' value  } <$ _)
instance Attr FeTurbulence_ UnicodeBidi (Event.Event Unit -> Event.Event  String ) where
  attr UnicodeBidi eventValue = unsafeAttribute (map (map (
    \value -> { key: "unicode-bidi", value: prop' value })) eventValue)
...
instance Attr Filter_ UnicodeBidi  String  where
  attr UnicodeBidi value = unsafeAttribute (  
    { key: "unicode-bidi", value: prop' value  } <$ _)
instance Attr Filter_ UnicodeBidi (Event.Event Unit -> Event.Event  String ) where
  attr UnicodeBidi eventValue = unsafeAttribute (map (map (
    \value -> { key: "unicode-bidi", value: prop' value })) eventValue)
...
instance Attr Image_ UnicodeBidi  String  where
  attr UnicodeBidi value = unsafeAttribute (  
    { key: "unicode-bidi", value: prop' value  } <$ _)
instance Attr Image_ UnicodeBidi (Event.Event Unit -> Event.Event  String ) where
  attr UnicodeBidi eventValue = unsafeAttribute (map (map (
    \value -> { key: "unicode-bidi", value: prop' value })) eventValue)
...
instance Attr Switch_ UnicodeBidi  String  where
  attr UnicodeBidi value = unsafeAttribute (  
    { key: "unicode-bidi", value: prop' value  } <$ _)
instance Attr Switch_ UnicodeBidi (Event.Event Unit -> Event.Event  String ) where
  attr UnicodeBidi eventValue = unsafeAttribute (map (map (
    \value -> { key: "unicode-bidi", value: prop' value })) eventValue)
...
instance Attr everything UnicodeBidi (Event.Event Unit -> Event.Event  Unit ) where
  attr UnicodeBidi eventValue = unsafeAttribute (map (map (
    \_ -> { key: "unicode-bidi", value: unset' })) eventValue)
