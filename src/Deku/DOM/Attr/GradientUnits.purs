...
instance Attr LinearGradient_ GradientUnits (Event.Event Unit -> Event.Event  String ) where
  attr GradientUnits eventValue = unsafeAttribute (map (map (
    \value -> { key: "gradientUnits", value: prop' value })) eventValue)
...
instance Attr RadialGradient_ GradientUnits  String  where
  attr GradientUnits value = unsafeAttribute (  
    { key: "gradientUnits", value: prop' value  } <$ _)
instance Attr RadialGradient_ GradientUnits (Event.Event Unit -> Event.Event  String ) where
  attr GradientUnits eventValue = unsafeAttribute (map (map (
    \value -> { key: "gradientUnits", value: prop' value })) eventValue)
...
instance Attr everything GradientUnits (Event.Event Unit -> Event.Event  Unit ) where
  attr GradientUnits eventValue = unsafeAttribute (map (map (
    \_ -> { key: "gradientUnits", value: unset' })) eventValue)
