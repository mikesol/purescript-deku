...
instance Attr FeBlend_ Clip  String  where
  attr Clip value = unsafeAttribute (  
    { key: "clip", value: prop' value  } <$ _)
instance Attr FeBlend_ Clip (Event.Event Unit -> Event.Event  String ) where
  attr Clip eventValue = unsafeAttribute (map (map ( \value ->
    { key: "clip", value: prop' value })) eventValue)
instance Attr FeBlend_ Clip (Event.Event  String ) where
  attr Clip eventValue = unsafeAttribute \_ -> eventValue <#> \value ->
    { key: "clip", value: prop' value }
instance Attr FeColorMatrix_ Clip  String  where
  attr Clip value = unsafeAttribute (  
    { key: "clip", value: prop' value  } <$ _)
instance Attr FeColorMatrix_ Clip (Event.Event Unit -> Event.Event  String ) where
  attr Clip eventValue = unsafeAttribute (map (map ( \value ->
    { key: "clip", value: prop' value })) eventValue)
...
instance Attr FeComponentTransfer_ Clip  String  where
  attr Clip value = unsafeAttribute (  
    { key: "clip", value: prop' value  } <$ _)
instance Attr FeComponentTransfer_ Clip (Event.Event Unit -> Event.Event  String ) where
  attr Clip eventValue = unsafeAttribute (map (map ( \value ->
    { key: "clip", value: prop' value })) eventValue)
...
instance Attr FeComposite_ Clip  String  where
  attr Clip value = unsafeAttribute (  
    { key: "clip", value: prop' value  } <$ _)
instance Attr FeComposite_ Clip (Event.Event Unit -> Event.Event  String ) where
  attr Clip eventValue = unsafeAttribute (map (map ( \value ->
    { key: "clip", value: prop' value })) eventValue)
...
instance Attr FeConvolveMatrix_ Clip  String  where
  attr Clip value = unsafeAttribute (  
    { key: "clip", value: prop' value  } <$ _)
instance Attr FeConvolveMatrix_ Clip (Event.Event Unit -> Event.Event  String ) where
  attr Clip eventValue = unsafeAttribute (map (map ( \value ->
    { key: "clip", value: prop' value })) eventValue)
...
instance Attr FeDiffuseLighting_ Clip  String  where
  attr Clip value = unsafeAttribute (  
    { key: "clip", value: prop' value  } <$ _)
instance Attr FeDiffuseLighting_ Clip (Event.Event Unit -> Event.Event  String ) where
  attr Clip eventValue = unsafeAttribute (map (map ( \value ->
    { key: "clip", value: prop' value })) eventValue)
...
instance Attr FeDisplacementMap_ Clip  String  where
  attr Clip value = unsafeAttribute (  
    { key: "clip", value: prop' value  } <$ _)
instance Attr FeDisplacementMap_ Clip (Event.Event Unit -> Event.Event  String ) where
  attr Clip eventValue = unsafeAttribute (map (map ( \value ->
    { key: "clip", value: prop' value })) eventValue)
...
instance Attr FeFlood_ Clip  String  where
  attr Clip value = unsafeAttribute (  
    { key: "clip", value: prop' value  } <$ _)
instance Attr FeFlood_ Clip (Event.Event Unit -> Event.Event  String ) where
  attr Clip eventValue = unsafeAttribute (map (map ( \value ->
    { key: "clip", value: prop' value })) eventValue)
...
instance Attr FeGaussianBlur_ Clip  String  where
  attr Clip value = unsafeAttribute (  
    { key: "clip", value: prop' value  } <$ _)
instance Attr FeGaussianBlur_ Clip (Event.Event Unit -> Event.Event  String ) where
  attr Clip eventValue = unsafeAttribute (map (map ( \value ->
    { key: "clip", value: prop' value })) eventValue)
...
instance Attr FeImage_ Clip  String  where
  attr Clip value = unsafeAttribute (  
    { key: "clip", value: prop' value  } <$ _)
instance Attr FeImage_ Clip (Event.Event Unit -> Event.Event  String ) where
  attr Clip eventValue = unsafeAttribute (map (map ( \value ->
    { key: "clip", value: prop' value })) eventValue)
...
instance Attr FeMerge_ Clip  String  where
  attr Clip value = unsafeAttribute (  
    { key: "clip", value: prop' value  } <$ _)
instance Attr FeMerge_ Clip (Event.Event Unit -> Event.Event  String ) where
  attr Clip eventValue = unsafeAttribute (map (map ( \value ->
    { key: "clip", value: prop' value })) eventValue)
...
instance Attr FeMorphology_ Clip  String  where
  attr Clip value = unsafeAttribute (  
    { key: "clip", value: prop' value  } <$ _)
instance Attr FeMorphology_ Clip (Event.Event Unit -> Event.Event  String ) where
  attr Clip eventValue = unsafeAttribute (map (map ( \value ->
    { key: "clip", value: prop' value })) eventValue)
...
instance Attr FeOffset_ Clip  String  where
  attr Clip value = unsafeAttribute (  
    { key: "clip", value: prop' value  } <$ _)
instance Attr FeOffset_ Clip (Event.Event Unit -> Event.Event  String ) where
  attr Clip eventValue = unsafeAttribute (map (map ( \value ->
    { key: "clip", value: prop' value })) eventValue)
...
instance Attr FeSpecularLighting_ Clip  String  where
  attr Clip value = unsafeAttribute (  
    { key: "clip", value: prop' value  } <$ _)
instance Attr FeSpecularLighting_ Clip (Event.Event Unit -> Event.Event  String ) where
  attr Clip eventValue = unsafeAttribute (map (map ( \value ->
    { key: "clip", value: prop' value })) eventValue)
...
instance Attr FeTile_ Clip  String  where
  attr Clip value = unsafeAttribute (  
    { key: "clip", value: prop' value  } <$ _)
instance Attr FeTile_ Clip (Event.Event Unit -> Event.Event  String ) where
  attr Clip eventValue = unsafeAttribute (map (map ( \value ->
    { key: "clip", value: prop' value })) eventValue)
...
instance Attr FeTurbulence_ Clip  String  where
  attr Clip value = unsafeAttribute (  
    { key: "clip", value: prop' value  } <$ _)
instance Attr FeTurbulence_ Clip (Event.Event Unit -> Event.Event  String ) where
  attr Clip eventValue = unsafeAttribute (map (map ( \value ->
    { key: "clip", value: prop' value })) eventValue)
...
instance Attr Filter_ Clip  String  where
  attr Clip value = unsafeAttribute (  
    { key: "clip", value: prop' value  } <$ _)
instance Attr Filter_ Clip (Event.Event Unit -> Event.Event  String ) where
  attr Clip eventValue = unsafeAttribute (map (map ( \value ->
    { key: "clip", value: prop' value })) eventValue)
...
instance Attr Image_ Clip  String  where
  attr Clip value = unsafeAttribute (  
    { key: "clip", value: prop' value  } <$ _)
instance Attr Image_ Clip (Event.Event Unit -> Event.Event  String ) where
  attr Clip eventValue = unsafeAttribute (map (map ( \value ->
    { key: "clip", value: prop' value })) eventValue)
...
instance Attr Switch_ Clip  String  where
  attr Clip value = unsafeAttribute (  
    { key: "clip", value: prop' value  } <$ _)
instance Attr Switch_ Clip (Event.Event Unit -> Event.Event  String ) where
  attr Clip eventValue = unsafeAttribute (map (map ( \value ->
    { key: "clip", value: prop' value })) eventValue)
...
instance Attr everything Clip  Unit  where
  attr Clip _ = unsafeAttribute (  { key: "clip", value: unset'  } <$ _)
instance Attr everything Clip (Event.Event Unit -> Event.Event  Unit ) where
  attr Clip eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "clip", value: unset' })) eventValue)
instance Attr everything Clip (Event.Event  Unit ) where
  attr Clip eventValue = unsafeAttribute \_ -> eventValue <#> \_ ->
