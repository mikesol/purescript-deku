...
instance Attr Circle_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr Circle_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
instance Attr Circle_ StrokeOpacity (Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute \_ -> eventValue <#>
    \value -> { key: "stroke-opacity", value: prop' value }
instance Attr ClipPath_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr ClipPath_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr Defs_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr Defs_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr Ellipse_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr Ellipse_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr FeBlend_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr FeBlend_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr FeColorMatrix_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr FeColorMatrix_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr FeComponentTransfer_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr FeComponentTransfer_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr FeComposite_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr FeComposite_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr FeConvolveMatrix_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr FeConvolveMatrix_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr FeDiffuseLighting_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr FeDiffuseLighting_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr FeDisplacementMap_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr FeDisplacementMap_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr FeFlood_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr FeFlood_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr FeGaussianBlur_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr FeGaussianBlur_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr FeImage_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr FeImage_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr FeMerge_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr FeMerge_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr FeMorphology_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr FeMorphology_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr FeOffset_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr FeOffset_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr FeSpecularLighting_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr FeSpecularLighting_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr FeTile_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr FeTile_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr FeTurbulence_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr FeTurbulence_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr Filter_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr Filter_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr ForeignObject_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr ForeignObject_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr G_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr G_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr Image_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr Image_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr Line_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr Line_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr LinearGradient_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr LinearGradient_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr Marker_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr Marker_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr Mask_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr Mask_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr Path_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr Path_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr Pattern_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr Pattern_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr Polygon_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr Polygon_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr Polyline_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr Polyline_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr RadialGradient_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr RadialGradient_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr Rect_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr Rect_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr Svg_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr Svg_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr Switch_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr Switch_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr Symbol_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr Symbol_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr Text_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr Text_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr TextPath_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr TextPath_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr Tspan_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr Tspan_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr Use_ StrokeOpacity  String  where
  attr StrokeOpacity value = unsafeAttribute (  
    { key: "stroke-opacity", value: prop' value  } <$ _)
instance Attr Use_ StrokeOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stroke-opacity", value: prop' value })) eventValue)
...
instance Attr everything StrokeOpacity  Unit  where
  attr StrokeOpacity _ = unsafeAttribute (  
    { key: "stroke-opacity", value: unset'  } <$ _)
instance Attr everything StrokeOpacity (Event.Event Unit -> Event.Event  Unit ) where
  attr StrokeOpacity eventValue = unsafeAttribute (map (map (
    \_ -> { key: "stroke-opacity", value: unset' })) eventValue)
