...
instance Attr FeComposite_ K4 (Event.Event Unit -> Event.Event  String ) where
  attr K4 eventValue = unsafeAttribute (map (map ( \value ->
    { key: "k4", value: prop' value })) eventValue)
...
instance Attr everything K4 (Event.Event Unit -> Event.Event  Unit ) where
  attr K4 eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "k4", value: unset' })) eventValue)
