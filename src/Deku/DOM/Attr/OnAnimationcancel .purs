...
instance Attr anything OnAnimationcancel  (Zora Unit) where
  attr OnAnimationcancel  value = unsafeAttribute { key: "animationcancel ", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnAnimationcancel  (Zora Boolean) where
  attr OnAnimationcancel  value = unsafeAttribute { key: "animationcancel ", value: cb' (Cb (const (runImpure value))) }