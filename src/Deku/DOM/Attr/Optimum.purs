...
instance Attr Meter_ Optimum (Event.Event Unit -> Event.Event  String ) where
  attr Optimum eventValue = unsafeAttribute (map (map (
    \value -> { key: "optimum", value: prop' value })) eventValue)
...
instance Attr everything Optimum (Event.Event Unit -> Event.Event  Unit ) where
  attr Optimum eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "optimum", value: unset' })) eventValue)
