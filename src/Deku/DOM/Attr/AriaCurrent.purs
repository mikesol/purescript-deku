...
instance Attr Circle_ AriaCurrent (Event.Event Unit -> Event.Event  String ) where
  attr AriaCurrent eventValue = unsafeAttribute (map (map (
    \value -> { key: "aria-current", value: prop' value })) eventValue)
...
instance Attr Ellipse_ AriaCurrent  String  where
  attr AriaCurrent value = unsafeAttribute (  
    { key: "aria-current", value: prop' value  } <$ _)
instance Attr Ellipse_ AriaCurrent (Event.Event Unit -> Event.Event  String ) where
  attr AriaCurrent eventValue = unsafeAttribute (map (map (
    \value -> { key: "aria-current", value: prop' value })) eventValue)
...
instance Attr ForeignObject_ AriaCurrent  String  where
  attr AriaCurrent value = unsafeAttribute (  
    { key: "aria-current", value: prop' value  } <$ _)
instance Attr ForeignObject_ AriaCurrent (Event.Event Unit -> Event.Event  String ) where
  attr AriaCurrent eventValue = unsafeAttribute (map (map (
    \value -> { key: "aria-current", value: prop' value })) eventValue)
...
instance Attr G_ AriaCurrent  String  where
  attr AriaCurrent value = unsafeAttribute (  
    { key: "aria-current", value: prop' value  } <$ _)
instance Attr G_ AriaCurrent (Event.Event Unit -> Event.Event  String ) where
  attr AriaCurrent eventValue = unsafeAttribute (map (map (
    \value -> { key: "aria-current", value: prop' value })) eventValue)
...
instance Attr Line_ AriaCurrent  String  where
  attr AriaCurrent value = unsafeAttribute (  
    { key: "aria-current", value: prop' value  } <$ _)
instance Attr Line_ AriaCurrent (Event.Event Unit -> Event.Event  String ) where
  attr AriaCurrent eventValue = unsafeAttribute (map (map (
    \value -> { key: "aria-current", value: prop' value })) eventValue)
...
instance Attr Marker_ AriaCurrent  String  where
  attr AriaCurrent value = unsafeAttribute (  
    { key: "aria-current", value: prop' value  } <$ _)
instance Attr Marker_ AriaCurrent (Event.Event Unit -> Event.Event  String ) where
  attr AriaCurrent eventValue = unsafeAttribute (map (map (
    \value -> { key: "aria-current", value: prop' value })) eventValue)
...
instance Attr Path_ AriaCurrent  String  where
  attr AriaCurrent value = unsafeAttribute (  
    { key: "aria-current", value: prop' value  } <$ _)
instance Attr Path_ AriaCurrent (Event.Event Unit -> Event.Event  String ) where
  attr AriaCurrent eventValue = unsafeAttribute (map (map (
    \value -> { key: "aria-current", value: prop' value })) eventValue)
...
instance Attr Polygon_ AriaCurrent  String  where
  attr AriaCurrent value = unsafeAttribute (  
    { key: "aria-current", value: prop' value  } <$ _)
instance Attr Polygon_ AriaCurrent (Event.Event Unit -> Event.Event  String ) where
  attr AriaCurrent eventValue = unsafeAttribute (map (map (
    \value -> { key: "aria-current", value: prop' value })) eventValue)
...
instance Attr Polyline_ AriaCurrent  String  where
  attr AriaCurrent value = unsafeAttribute (  
    { key: "aria-current", value: prop' value  } <$ _)
instance Attr Polyline_ AriaCurrent (Event.Event Unit -> Event.Event  String ) where
  attr AriaCurrent eventValue = unsafeAttribute (map (map (
    \value -> { key: "aria-current", value: prop' value })) eventValue)
...
instance Attr Rect_ AriaCurrent  String  where
  attr AriaCurrent value = unsafeAttribute (  
    { key: "aria-current", value: prop' value  } <$ _)
instance Attr Rect_ AriaCurrent (Event.Event Unit -> Event.Event  String ) where
  attr AriaCurrent eventValue = unsafeAttribute (map (map (
    \value -> { key: "aria-current", value: prop' value })) eventValue)
...
instance Attr Svg_ AriaCurrent  String  where
  attr AriaCurrent value = unsafeAttribute (  
    { key: "aria-current", value: prop' value  } <$ _)
instance Attr Svg_ AriaCurrent (Event.Event Unit -> Event.Event  String ) where
  attr AriaCurrent eventValue = unsafeAttribute (map (map (
    \value -> { key: "aria-current", value: prop' value })) eventValue)
...
instance Attr Symbol_ AriaCurrent  String  where
  attr AriaCurrent value = unsafeAttribute (  
    { key: "aria-current", value: prop' value  } <$ _)
instance Attr Symbol_ AriaCurrent (Event.Event Unit -> Event.Event  String ) where
  attr AriaCurrent eventValue = unsafeAttribute (map (map (
    \value -> { key: "aria-current", value: prop' value })) eventValue)
...
instance Attr Text_ AriaCurrent  String  where
  attr AriaCurrent value = unsafeAttribute (  
    { key: "aria-current", value: prop' value  } <$ _)
instance Attr Text_ AriaCurrent (Event.Event Unit -> Event.Event  String ) where
  attr AriaCurrent eventValue = unsafeAttribute (map (map (
    \value -> { key: "aria-current", value: prop' value })) eventValue)
...
instance Attr TextPath_ AriaCurrent  String  where
  attr AriaCurrent value = unsafeAttribute (  
    { key: "aria-current", value: prop' value  } <$ _)
instance Attr TextPath_ AriaCurrent (Event.Event Unit -> Event.Event  String ) where
  attr AriaCurrent eventValue = unsafeAttribute (map (map (
    \value -> { key: "aria-current", value: prop' value })) eventValue)
...
instance Attr Tspan_ AriaCurrent  String  where
  attr AriaCurrent value = unsafeAttribute (  
    { key: "aria-current", value: prop' value  } <$ _)
instance Attr Tspan_ AriaCurrent (Event.Event Unit -> Event.Event  String ) where
  attr AriaCurrent eventValue = unsafeAttribute (map (map (
    \value -> { key: "aria-current", value: prop' value })) eventValue)
...
instance Attr Use_ AriaCurrent  String  where
  attr AriaCurrent value = unsafeAttribute (  
    { key: "aria-current", value: prop' value  } <$ _)
instance Attr Use_ AriaCurrent (Event.Event Unit -> Event.Event  String ) where
  attr AriaCurrent eventValue = unsafeAttribute (map (map (
    \value -> { key: "aria-current", value: prop' value })) eventValue)
...
instance Attr View_ AriaCurrent  String  where
  attr AriaCurrent value = unsafeAttribute (  
    { key: "aria-current", value: prop' value  } <$ _)
instance Attr View_ AriaCurrent (Event.Event Unit -> Event.Event  String ) where
  attr AriaCurrent eventValue = unsafeAttribute (map (map (
    \value -> { key: "aria-current", value: prop' value })) eventValue)
...
instance Attr everything AriaCurrent (Event.Event Unit -> Event.Event  Unit ) where
  attr AriaCurrent eventValue = unsafeAttribute (map (map (
    \_ -> { key: "aria-current", value: unset' })) eventValue)
