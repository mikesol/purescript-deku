...
instance Attr anything OnPointerenter (Event.Event Unit -> Event.Event  Cb ) where
  attr OnPointerenter eventValue = unsafeAttribute (map (map (
    \value -> { key: "pointerenter", value: cb' value })) eventValue)
...
instance Attr anything OnPointerenter (Event.Event Unit -> Event.Event  (Effect Unit) ) where
  attr OnPointerenter eventValue = unsafeAttribute (map (map (
    \value -> { key: "pointerenter", value: cb' (Cb (const (value $> true))) })) eventValue)
...
instance Attr anything OnPointerenter (Event.Event Unit -> Event.Event  (Effect Boolean) ) where
  attr OnPointerenter eventValue = unsafeAttribute (map (map (
    \value -> { key: "pointerenter", value: cb' (Cb (const value)) })) eventValue) 
...
instance Attr everything OnPointerenter (Event.Event Unit -> Event.Event  Unit ) where
  attr OnPointerenter eventValue = unsafeAttribute (map (map (
    \_ -> { key: "pointerenter", value: unset' })) eventValue)
