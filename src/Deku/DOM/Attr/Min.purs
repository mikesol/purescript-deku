...
instance Attr Input_ Min (Event.Event Unit -> Event.Event  String ) where
  attr Min eventValue = unsafeAttribute (map (map ( \value ->
    { key: "min", value: prop' value })) eventValue)
...
instance Attr Meter_ Min  String  where
  attr Min value = unsafeAttribute (  { key: "min", value: prop' value  } <$ _)
instance Attr Meter_ Min (Event.Event Unit -> Event.Event  String ) where
  attr Min eventValue = unsafeAttribute (map (map ( \value ->
    { key: "min", value: prop' value })) eventValue)
...
instance Attr Animate_ Min  String  where
  attr Min value = unsafeAttribute (  { key: "min", value: prop' value  } <$ _)
instance Attr Animate_ Min (Event.Event Unit -> Event.Event  String ) where
  attr Min eventValue = unsafeAttribute (map (map ( \value ->
    { key: "min", value: prop' value })) eventValue)
...
instance Attr AnimateMotion_ Min  String  where
  attr Min value = unsafeAttribute (  { key: "min", value: prop' value  } <$ _)
instance Attr AnimateMotion_ Min (Event.Event Unit -> Event.Event  String ) where
  attr Min eventValue = unsafeAttribute (map (map ( \value ->
    { key: "min", value: prop' value })) eventValue)
...
instance Attr AnimateTransform_ Min  String  where
  attr Min value = unsafeAttribute (  { key: "min", value: prop' value  } <$ _)
instance Attr AnimateTransform_ Min (Event.Event Unit -> Event.Event  String ) where
  attr Min eventValue = unsafeAttribute (map (map ( \value ->
    { key: "min", value: prop' value })) eventValue)
...
instance Attr Set_ Min  String  where
  attr Min value = unsafeAttribute (  { key: "min", value: prop' value  } <$ _)
instance Attr Set_ Min (Event.Event Unit -> Event.Event  String ) where
  attr Min eventValue = unsafeAttribute (map (map ( \value ->
    { key: "min", value: prop' value })) eventValue)
...
instance Attr everything Min (Event.Event Unit -> Event.Event  Unit ) where
  attr Min eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "min", value: unset' })) eventValue)
