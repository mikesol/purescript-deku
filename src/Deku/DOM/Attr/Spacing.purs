...
instance Attr TextPath_ Spacing (Event.Event Unit -> Event.Event  String ) where
  attr Spacing eventValue = unsafeAttribute (map (map (
    \value -> { key: "spacing", value: prop' value })) eventValue)
...
instance Attr everything Spacing (Event.Event Unit -> Event.Event  Unit ) where
  attr Spacing eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "spacing", value: unset' })) eventValue)
