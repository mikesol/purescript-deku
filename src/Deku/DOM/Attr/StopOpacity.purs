...
instance Attr FeBlend_ StopOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StopOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-opacity", value: prop' value })) eventValue)
...
instance Attr FeColorMatrix_ StopOpacity  String  where
  attr StopOpacity value = unsafeAttribute (  
    { key: "stop-opacity", value: prop' value  } <$ _)
instance Attr FeColorMatrix_ StopOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StopOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-opacity", value: prop' value })) eventValue)
...
instance Attr FeComponentTransfer_ StopOpacity  String  where
  attr StopOpacity value = unsafeAttribute (  
    { key: "stop-opacity", value: prop' value  } <$ _)
instance Attr FeComponentTransfer_ StopOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StopOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-opacity", value: prop' value })) eventValue)
...
instance Attr FeComposite_ StopOpacity  String  where
  attr StopOpacity value = unsafeAttribute (  
    { key: "stop-opacity", value: prop' value  } <$ _)
instance Attr FeComposite_ StopOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StopOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-opacity", value: prop' value })) eventValue)
...
instance Attr FeConvolveMatrix_ StopOpacity  String  where
  attr StopOpacity value = unsafeAttribute (  
    { key: "stop-opacity", value: prop' value  } <$ _)
instance Attr FeConvolveMatrix_ StopOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StopOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-opacity", value: prop' value })) eventValue)
...
instance Attr FeDiffuseLighting_ StopOpacity  String  where
  attr StopOpacity value = unsafeAttribute (  
    { key: "stop-opacity", value: prop' value  } <$ _)
instance Attr FeDiffuseLighting_ StopOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StopOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-opacity", value: prop' value })) eventValue)
...
instance Attr FeDisplacementMap_ StopOpacity  String  where
  attr StopOpacity value = unsafeAttribute (  
    { key: "stop-opacity", value: prop' value  } <$ _)
instance Attr FeDisplacementMap_ StopOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StopOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-opacity", value: prop' value })) eventValue)
...
instance Attr FeFlood_ StopOpacity  String  where
  attr StopOpacity value = unsafeAttribute (  
    { key: "stop-opacity", value: prop' value  } <$ _)
instance Attr FeFlood_ StopOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StopOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-opacity", value: prop' value })) eventValue)
...
instance Attr FeGaussianBlur_ StopOpacity  String  where
  attr StopOpacity value = unsafeAttribute (  
    { key: "stop-opacity", value: prop' value  } <$ _)
instance Attr FeGaussianBlur_ StopOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StopOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-opacity", value: prop' value })) eventValue)
...
instance Attr FeImage_ StopOpacity  String  where
  attr StopOpacity value = unsafeAttribute (  
    { key: "stop-opacity", value: prop' value  } <$ _)
instance Attr FeImage_ StopOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StopOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-opacity", value: prop' value })) eventValue)
...
instance Attr FeMerge_ StopOpacity  String  where
  attr StopOpacity value = unsafeAttribute (  
    { key: "stop-opacity", value: prop' value  } <$ _)
instance Attr FeMerge_ StopOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StopOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-opacity", value: prop' value })) eventValue)
...
instance Attr FeMorphology_ StopOpacity  String  where
  attr StopOpacity value = unsafeAttribute (  
    { key: "stop-opacity", value: prop' value  } <$ _)
instance Attr FeMorphology_ StopOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StopOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-opacity", value: prop' value })) eventValue)
...
instance Attr FeOffset_ StopOpacity  String  where
  attr StopOpacity value = unsafeAttribute (  
    { key: "stop-opacity", value: prop' value  } <$ _)
instance Attr FeOffset_ StopOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StopOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-opacity", value: prop' value })) eventValue)
...
instance Attr FeSpecularLighting_ StopOpacity  String  where
  attr StopOpacity value = unsafeAttribute (  
    { key: "stop-opacity", value: prop' value  } <$ _)
instance Attr FeSpecularLighting_ StopOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StopOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-opacity", value: prop' value })) eventValue)
...
instance Attr FeTile_ StopOpacity  String  where
  attr StopOpacity value = unsafeAttribute (  
    { key: "stop-opacity", value: prop' value  } <$ _)
instance Attr FeTile_ StopOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StopOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-opacity", value: prop' value })) eventValue)
...
instance Attr FeTurbulence_ StopOpacity  String  where
  attr StopOpacity value = unsafeAttribute (  
    { key: "stop-opacity", value: prop' value  } <$ _)
instance Attr FeTurbulence_ StopOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StopOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-opacity", value: prop' value })) eventValue)
...
instance Attr Filter_ StopOpacity  String  where
  attr StopOpacity value = unsafeAttribute (  
    { key: "stop-opacity", value: prop' value  } <$ _)
instance Attr Filter_ StopOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StopOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-opacity", value: prop' value })) eventValue)
...
instance Attr Image_ StopOpacity  String  where
  attr StopOpacity value = unsafeAttribute (  
    { key: "stop-opacity", value: prop' value  } <$ _)
instance Attr Image_ StopOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StopOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-opacity", value: prop' value })) eventValue)
...
instance Attr Stop_ StopOpacity  String  where
  attr StopOpacity value = unsafeAttribute (  
    { key: "stop-opacity", value: prop' value  } <$ _)
instance Attr Stop_ StopOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StopOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-opacity", value: prop' value })) eventValue)
...
instance Attr Switch_ StopOpacity  String  where
  attr StopOpacity value = unsafeAttribute (  
    { key: "stop-opacity", value: prop' value  } <$ _)
instance Attr Switch_ StopOpacity (Event.Event Unit -> Event.Event  String ) where
  attr StopOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-opacity", value: prop' value })) eventValue)
...
instance Attr everything StopOpacity (Event.Event Unit -> Event.Event  Unit ) where
  attr StopOpacity eventValue = unsafeAttribute (map (map (
    \_ -> { key: "stop-opacity", value: unset' })) eventValue)
