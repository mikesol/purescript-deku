...
instance Attr anything OnLoad (Event.Event Unit -> Event.Event  Cb ) where
  attr OnLoad eventValue = unsafeAttribute (map (map (
    \value -> { key: "load", value: cb' value })) eventValue)
...
instance Attr anything OnLoad (Event.Event Unit -> Event.Event  (Effect Unit) ) where
  attr OnLoad eventValue = unsafeAttribute (map (map (
    \value -> { key: "load", value: cb' (Cb (const (value $> true))) })) eventValue)
...
instance Attr anything OnLoad (Event.Event Unit -> Event.Event  (Effect Boolean) ) where
  attr OnLoad eventValue = unsafeAttribute (map (map (
    \value -> { key: "load", value: cb' (Cb (const value)) })) eventValue) 
...
instance Attr everything OnLoad (Event.Event Unit -> Event.Event  Unit ) where
  attr OnLoad eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "load", value: unset' })) eventValue)
