...
instance Attr anything OnSubmit (Event.Event Unit -> Event.Event  Cb ) where
  attr OnSubmit eventValue = unsafeAttribute (map (map (
    \value -> { key: "submit", value: cb' value })) eventValue)
...
instance Attr anything OnSubmit (Event.Event Unit -> Event.Event  (Effect Unit) ) where
  attr OnSubmit eventValue = unsafeAttribute (map (map (
    \value -> { key: "submit", value: cb' (Cb (const (value $> true))) })) eventValue)
...
instance Attr anything OnSubmit (Event.Event Unit -> Event.Event  (Effect Boolean) ) where
  attr OnSubmit eventValue = unsafeAttribute (map (map (
    \value -> { key: "submit", value: cb' (Cb (const value)) })) eventValue) 
...
instance Attr everything OnSubmit (Event.Event Unit -> Event.Event  Unit ) where
  attr OnSubmit eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "submit", value: unset' })) eventValue)
