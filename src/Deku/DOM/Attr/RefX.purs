...
instance Attr Marker_ RefX (Event.Event Unit -> Event.Event  String ) where
  attr RefX eventValue = unsafeAttribute (map (map ( \value ->
    { key: "refX", value: prop' value })) eventValue)
...
instance Attr Symbol_ RefX  String  where
  attr RefX value = unsafeAttribute (  
    { key: "refX", value: prop' value  } <$ _)
instance Attr Symbol_ RefX (Event.Event Unit -> Event.Event  String ) where
  attr RefX eventValue = unsafeAttribute (map (map ( \value ->
    { key: "refX", value: prop' value })) eventValue)
...
instance Attr everything RefX (Event.Event Unit -> Event.Event  Unit ) where
  attr RefX eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "refX", value: unset' })) eventValue)
