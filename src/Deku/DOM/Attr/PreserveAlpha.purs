...
instance Attr FeConvolveMatrix_ PreserveAlpha (Event.Event Unit -> Event.Event  String ) where
  attr PreserveAlpha eventValue = unsafeAttribute (map (map (
    \value -> { key: "preserveAlpha", value: prop' value })) eventValue)
...
instance Attr everything PreserveAlpha (Event.Event Unit -> Event.Event  Unit ) where
  attr PreserveAlpha eventValue = unsafeAttribute (map (map (
    \_ -> { key: "preserveAlpha", value: unset' })) eventValue)
