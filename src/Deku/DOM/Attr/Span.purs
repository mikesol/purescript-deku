...
instance Attr Col_ Span (Event.Event Unit -> Event.Event  String ) where
  attr Span eventValue = unsafeAttribute (map (map ( \value ->
    { key: "span", value: prop' value })) eventValue)
...
instance Attr Colgroup_ Span  String  where
  attr Span value = unsafeAttribute (  
    { key: "span", value: prop' value  } <$ _)
instance Attr Colgroup_ Span (Event.Event Unit -> Event.Event  String ) where
  attr Span eventValue = unsafeAttribute (map (map ( \value ->
    { key: "span", value: prop' value })) eventValue)
...
instance Attr everything Span (Event.Event Unit -> Event.Event  Unit ) where
  attr Span eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "span", value: unset' })) eventValue)
