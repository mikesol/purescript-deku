...
instance Attr Pattern_ PatternTransform (Event.Event Unit -> Event.Event  String ) where
  attr PatternTransform eventValue = unsafeAttribute (map (map ( \value -> { key: "patternTransform", value: prop' value })) eventValue)
...
instance Attr everything PatternTransform (Event.Event Unit -> Event.Event  Unit ) where
  attr PatternTransform eventValue = unsafeAttribute (map (map ( \_ -> { key: "patternTransform", value: unset' })) eventValue)
