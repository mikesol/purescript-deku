...
instance Attr Button_ Formnovalidate (Event.Event Unit -> Event.Event  String ) where
  attr Formnovalidate eventValue = unsafeAttribute (map (map (
    \value -> { key: "formnovalidate", value: prop' value })) eventValue)
...
instance Attr Input_ Formnovalidate  String  where
  attr Formnovalidate value = unsafeAttribute (  
    { key: "formnovalidate", value: prop' value  } <$ _)
instance Attr Input_ Formnovalidate (Event.Event Unit -> Event.Event  String ) where
  attr Formnovalidate eventValue = unsafeAttribute (map (map (
    \value -> { key: "formnovalidate", value: prop' value })) eventValue)
...
instance Attr everything Formnovalidate (Event.Event Unit -> Event.Event  Unit ) where
  attr Formnovalidate eventValue = unsafeAttribute (map (map (
    \_ -> { key: "formnovalidate", value: unset' })) eventValue)
