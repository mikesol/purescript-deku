...
instance Attr Table_ Summary (Event.Event Unit -> Event.Event  String ) where
  attr Summary eventValue = unsafeAttribute (map (map (
    \value -> { key: "summary", value: prop' value })) eventValue)
...
instance Attr everything Summary (Event.Event Unit -> Event.Event  Unit ) where
  attr Summary eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "summary", value: unset' })) eventValue)
