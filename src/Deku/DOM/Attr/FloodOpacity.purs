...
instance Attr FeBlend_ FloodOpacity (Event.Event Unit -> Event.Event  String ) where
  attr FloodOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "flood-opacity", value: prop' value })) eventValue)
...
instance Attr FeColorMatrix_ FloodOpacity  String  where
  attr FloodOpacity value = unsafeAttribute (  
    { key: "flood-opacity", value: prop' value  } <$ _)
instance Attr FeColorMatrix_ FloodOpacity (Event.Event Unit -> Event.Event  String ) where
  attr FloodOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "flood-opacity", value: prop' value })) eventValue)
...
instance Attr FeComponentTransfer_ FloodOpacity  String  where
  attr FloodOpacity value = unsafeAttribute (  
    { key: "flood-opacity", value: prop' value  } <$ _)
instance Attr FeComponentTransfer_ FloodOpacity (Event.Event Unit -> Event.Event  String ) where
  attr FloodOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "flood-opacity", value: prop' value })) eventValue)
...
instance Attr FeComposite_ FloodOpacity  String  where
  attr FloodOpacity value = unsafeAttribute (  
    { key: "flood-opacity", value: prop' value  } <$ _)
instance Attr FeComposite_ FloodOpacity (Event.Event Unit -> Event.Event  String ) where
  attr FloodOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "flood-opacity", value: prop' value })) eventValue)
...
instance Attr FeConvolveMatrix_ FloodOpacity  String  where
  attr FloodOpacity value = unsafeAttribute (  
    { key: "flood-opacity", value: prop' value  } <$ _)
instance Attr FeConvolveMatrix_ FloodOpacity (Event.Event Unit -> Event.Event  String ) where
  attr FloodOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "flood-opacity", value: prop' value })) eventValue)
...
instance Attr FeDiffuseLighting_ FloodOpacity  String  where
  attr FloodOpacity value = unsafeAttribute (  
    { key: "flood-opacity", value: prop' value  } <$ _)
instance Attr FeDiffuseLighting_ FloodOpacity (Event.Event Unit -> Event.Event  String ) where
  attr FloodOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "flood-opacity", value: prop' value })) eventValue)
...
instance Attr FeDisplacementMap_ FloodOpacity  String  where
  attr FloodOpacity value = unsafeAttribute (  
    { key: "flood-opacity", value: prop' value  } <$ _)
instance Attr FeDisplacementMap_ FloodOpacity (Event.Event Unit -> Event.Event  String ) where
  attr FloodOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "flood-opacity", value: prop' value })) eventValue)
...
instance Attr FeDropShadow_ FloodOpacity  String  where
  attr FloodOpacity value = unsafeAttribute (  
    { key: "flood-opacity", value: prop' value  } <$ _)
instance Attr FeDropShadow_ FloodOpacity (Event.Event Unit -> Event.Event  String ) where
  attr FloodOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "flood-opacity", value: prop' value })) eventValue)
...
instance Attr FeFlood_ FloodOpacity  String  where
  attr FloodOpacity value = unsafeAttribute (  
    { key: "flood-opacity", value: prop' value  } <$ _)
instance Attr FeFlood_ FloodOpacity (Event.Event Unit -> Event.Event  String ) where
  attr FloodOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "flood-opacity", value: prop' value })) eventValue)
...
instance Attr FeGaussianBlur_ FloodOpacity  String  where
  attr FloodOpacity value = unsafeAttribute (  
    { key: "flood-opacity", value: prop' value  } <$ _)
instance Attr FeGaussianBlur_ FloodOpacity (Event.Event Unit -> Event.Event  String ) where
  attr FloodOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "flood-opacity", value: prop' value })) eventValue)
...
instance Attr FeImage_ FloodOpacity  String  where
  attr FloodOpacity value = unsafeAttribute (  
    { key: "flood-opacity", value: prop' value  } <$ _)
instance Attr FeImage_ FloodOpacity (Event.Event Unit -> Event.Event  String ) where
  attr FloodOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "flood-opacity", value: prop' value })) eventValue)
...
instance Attr FeMerge_ FloodOpacity  String  where
  attr FloodOpacity value = unsafeAttribute (  
    { key: "flood-opacity", value: prop' value  } <$ _)
instance Attr FeMerge_ FloodOpacity (Event.Event Unit -> Event.Event  String ) where
  attr FloodOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "flood-opacity", value: prop' value })) eventValue)
...
instance Attr FeMorphology_ FloodOpacity  String  where
  attr FloodOpacity value = unsafeAttribute (  
    { key: "flood-opacity", value: prop' value  } <$ _)
instance Attr FeMorphology_ FloodOpacity (Event.Event Unit -> Event.Event  String ) where
  attr FloodOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "flood-opacity", value: prop' value })) eventValue)
...
instance Attr FeOffset_ FloodOpacity  String  where
  attr FloodOpacity value = unsafeAttribute (  
    { key: "flood-opacity", value: prop' value  } <$ _)
instance Attr FeOffset_ FloodOpacity (Event.Event Unit -> Event.Event  String ) where
  attr FloodOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "flood-opacity", value: prop' value })) eventValue)
...
instance Attr FeSpecularLighting_ FloodOpacity  String  where
  attr FloodOpacity value = unsafeAttribute (  
    { key: "flood-opacity", value: prop' value  } <$ _)
instance Attr FeSpecularLighting_ FloodOpacity (Event.Event Unit -> Event.Event  String ) where
  attr FloodOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "flood-opacity", value: prop' value })) eventValue)
...
instance Attr FeTile_ FloodOpacity  String  where
  attr FloodOpacity value = unsafeAttribute (  
    { key: "flood-opacity", value: prop' value  } <$ _)
instance Attr FeTile_ FloodOpacity (Event.Event Unit -> Event.Event  String ) where
  attr FloodOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "flood-opacity", value: prop' value })) eventValue)
...
instance Attr FeTurbulence_ FloodOpacity  String  where
  attr FloodOpacity value = unsafeAttribute (  
    { key: "flood-opacity", value: prop' value  } <$ _)
instance Attr FeTurbulence_ FloodOpacity (Event.Event Unit -> Event.Event  String ) where
  attr FloodOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "flood-opacity", value: prop' value })) eventValue)
...
instance Attr Filter_ FloodOpacity  String  where
  attr FloodOpacity value = unsafeAttribute (  
    { key: "flood-opacity", value: prop' value  } <$ _)
instance Attr Filter_ FloodOpacity (Event.Event Unit -> Event.Event  String ) where
  attr FloodOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "flood-opacity", value: prop' value })) eventValue)
...
instance Attr Image_ FloodOpacity  String  where
  attr FloodOpacity value = unsafeAttribute (  
    { key: "flood-opacity", value: prop' value  } <$ _)
instance Attr Image_ FloodOpacity (Event.Event Unit -> Event.Event  String ) where
  attr FloodOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "flood-opacity", value: prop' value })) eventValue)
...
instance Attr Switch_ FloodOpacity  String  where
  attr FloodOpacity value = unsafeAttribute (  
    { key: "flood-opacity", value: prop' value  } <$ _)
instance Attr Switch_ FloodOpacity (Event.Event Unit -> Event.Event  String ) where
  attr FloodOpacity eventValue = unsafeAttribute (map (map (
    \value -> { key: "flood-opacity", value: prop' value })) eventValue)
...
instance Attr everything FloodOpacity (Event.Event Unit -> Event.Event  Unit ) where
  attr FloodOpacity eventValue = unsafeAttribute (map (map (
    \_ -> { key: "flood-opacity", value: unset' })) eventValue)
