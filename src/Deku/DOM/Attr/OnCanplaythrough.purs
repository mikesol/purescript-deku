...
instance Attr anything OnCanplaythrough (Zora Unit) where
  attr OnCanplaythrough value = unsafeAttribute { key: "canplaythrough", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnCanplaythrough (Zora Boolean) where
  attr OnCanplaythrough value = unsafeAttribute { key: "canplaythrough", value: cb' (Cb (const (runImpure value))) }