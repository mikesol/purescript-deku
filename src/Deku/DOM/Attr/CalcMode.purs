...
instance Attr Animate_ CalcMode (Event.Event Unit -> Event.Event  String ) where
  attr CalcMode eventValue = unsafeAttribute (map (map (
    \value -> { key: "calcMode", value: prop' value })) eventValue)
...
instance Attr AnimateMotion_ CalcMode  String  where
  attr CalcMode value = unsafeAttribute (  
    { key: "calcMode", value: prop' value  } <$ _)
instance Attr AnimateMotion_ CalcMode (Event.Event Unit -> Event.Event  String ) where
  attr CalcMode eventValue = unsafeAttribute (map (map (
    \value -> { key: "calcMode", value: prop' value })) eventValue)
...
instance Attr AnimateTransform_ CalcMode  String  where
  attr CalcMode value = unsafeAttribute (  
    { key: "calcMode", value: prop' value  } <$ _)
instance Attr AnimateTransform_ CalcMode (Event.Event Unit -> Event.Event  String ) where
  attr CalcMode eventValue = unsafeAttribute (map (map (
    \value -> { key: "calcMode", value: prop' value })) eventValue)
...
instance Attr everything CalcMode (Event.Event Unit -> Event.Event  Unit ) where
  attr CalcMode eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "calcMode", value: unset' })) eventValue)
