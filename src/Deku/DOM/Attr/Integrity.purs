...
instance Attr Link_ Integrity (Event.Event Unit -> Event.Event  String ) where
  attr Integrity eventValue = unsafeAttribute (map (map (
    \value -> { key: "integrity", value: prop' value })) eventValue)
...
instance Attr Script_ Integrity  String  where
  attr Integrity value = unsafeAttribute (  
    { key: "integrity", value: prop' value  } <$ _)
instance Attr Script_ Integrity (Event.Event Unit -> Event.Event  String ) where
  attr Integrity eventValue = unsafeAttribute (map (map (
    \value -> { key: "integrity", value: prop' value })) eventValue)
...
instance Attr everything Integrity (Event.Event Unit -> Event.Event  Unit ) where
  attr Integrity eventValue = unsafeAttribute (map (map (
    \_ -> { key: "integrity", value: unset' })) eventValue)
