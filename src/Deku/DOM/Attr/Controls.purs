...
instance Attr Audio_ Controls (Event.Event Unit -> Event.Event  String ) where
  attr Controls eventValue = unsafeAttribute (map (map (
    \value -> { key: "controls", value: prop' value })) eventValue)
...
instance Attr Video_ Controls  String  where
  attr Controls value = unsafeAttribute (  
    { key: "controls", value: prop' value  } <$ _)
instance Attr Video_ Controls (Event.Event Unit -> Event.Event  String ) where
  attr Controls eventValue = unsafeAttribute (map (map (
    \value -> { key: "controls", value: prop' value })) eventValue)
...
instance Attr everything Controls (Event.Event Unit -> Event.Event  Unit ) where
  attr Controls eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "controls", value: unset' })) eventValue)
