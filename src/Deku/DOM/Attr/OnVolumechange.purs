...
instance Attr anything OnVolumechange (Event.Event Unit -> Event.Event  Cb ) where
  attr OnVolumechange eventValue = unsafeAttribute (map (map (
    \value -> { key: "volumechange", value: cb' value })) eventValue)
...
instance Attr anything OnVolumechange (Event.Event Unit -> Event.Event  (Effect Unit) ) where
  attr OnVolumechange eventValue = unsafeAttribute (map (map (
    \value -> { key: "volumechange", value: cb' (Cb (const (value $> true))) })) eventValue)
...
instance Attr anything OnVolumechange (Event.Event Unit -> Event.Event  (Effect Boolean) ) where
  attr OnVolumechange eventValue = unsafeAttribute (map (map (
    \value -> { key: "volumechange", value: cb' (Cb (const value)) })) eventValue) 
...
instance Attr everything OnVolumechange (Event.Event Unit -> Event.Event  Unit ) where
  attr OnVolumechange eventValue = unsafeAttribute (map (map (
    \_ -> { key: "volumechange", value: unset' })) eventValue)
