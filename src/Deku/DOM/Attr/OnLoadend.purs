...
instance Attr anything OnLoadend (Zora Unit) where
  attr OnLoadend value = unsafeAttribute { key: "loadend", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnLoadend (Zora Boolean) where
  attr OnLoadend value = unsafeAttribute { key: "loadend", value: cb' (Cb (const (runImpure value))) }