...
instance Attr FeConvolveMatrix_ Bias (Event.Event Unit -> Event.Event  String ) where
  attr Bias eventValue = unsafeAttribute (map (map ( \value ->
    { key: "bias", value: prop' value })) eventValue)
...
instance Attr everything Bias (Event.Event Unit -> Event.Event  Unit ) where
  attr Bias eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "bias", value: unset' })) eventValue)
