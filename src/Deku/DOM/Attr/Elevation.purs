...
instance Attr FeDistantLight_ Elevation (Event.Event Unit -> Event.Event  String ) where
  attr Elevation eventValue = unsafeAttribute (map (map (
    \value -> { key: "elevation", value: prop' value })) eventValue)
...
instance Attr everything Elevation (Event.Event Unit -> Event.Event  Unit ) where
  attr Elevation eventValue = unsafeAttribute (map (map (
    \_ -> { key: "elevation", value: unset' })) eventValue)
