...
instance Attr Pattern_ PatternContentUnits (Event.Event Unit -> Event.Event  String ) where
  attr PatternContentUnits eventValue = unsafeAttribute (map (map ( \value -> { key: "patternContentUnits", value: prop' value })) eventValue)
...
instance Attr everything PatternContentUnits (Event.Event Unit -> Event.Event  Unit ) where
  attr PatternContentUnits eventValue = unsafeAttribute (map (map ( \_ -> { key: "patternContentUnits", value: unset' })) eventValue)
