...
instance Attr RadialGradient_ Fy (Event.Event Unit -> Event.Event  String ) where
  attr Fy eventValue = unsafeAttribute (map (map ( \value ->
    { key: "fy", value: prop' value })) eventValue)
...
instance Attr everything Fy (Event.Event Unit -> Event.Event  Unit ) where
  attr Fy eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "fy", value: unset' })) eventValue)
