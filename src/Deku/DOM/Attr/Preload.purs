...
instance Attr Audio_ Preload (Event.Event Unit -> Event.Event  String ) where
  attr Preload eventValue = unsafeAttribute (map (map (
    \value -> { key: "preload", value: prop' value })) eventValue)
...
instance Attr Video_ Preload  String  where
  attr Preload value = unsafeAttribute (  
    { key: "preload", value: prop' value  } <$ _)
instance Attr Video_ Preload (Event.Event Unit -> Event.Event  String ) where
  attr Preload eventValue = unsafeAttribute (map (map (
    \value -> { key: "preload", value: prop' value })) eventValue)
...
instance Attr everything Preload (Event.Event Unit -> Event.Event  Unit ) where
  attr Preload eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "preload", value: unset' })) eventValue)
