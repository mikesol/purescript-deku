...
instance Attr anything OnTimeupdate (Event.Event Unit -> Event.Event  Cb ) where
  attr OnTimeupdate eventValue = unsafeAttribute (map (map (
    \value -> { key: "timeupdate", value: cb' value })) eventValue)
...
instance Attr anything OnTimeupdate (Event.Event Unit -> Event.Event  (Effect Unit) ) where
  attr OnTimeupdate eventValue = unsafeAttribute (map (map (
    \value -> { key: "timeupdate", value: cb' (Cb (const (value $> true))) })) eventValue)
...
instance Attr anything OnTimeupdate (Event.Event Unit -> Event.Event  (Effect Boolean) ) where
  attr OnTimeupdate eventValue = unsafeAttribute (map (map (
    \value -> { key: "timeupdate", value: cb' (Cb (const value)) })) eventValue) 
...
instance Attr everything OnTimeupdate (Event.Event Unit -> Event.Event  Unit ) where
  attr OnTimeupdate eventValue = unsafeAttribute (map (map (
    \_ -> { key: "timeupdate", value: unset' })) eventValue)
