...
instance Attr anything OnAuxclick (Event.Event Unit -> Event.Event  Cb ) where
  attr OnAuxclick eventValue = unsafeAttribute (map (map (
    \value -> { key: "auxclick", value: cb' value })) eventValue)
...
instance Attr anything OnAuxclick (Event.Event Unit -> Event.Event  (Effect Unit) ) where
  attr OnAuxclick eventValue = unsafeAttribute (map (map (
    \value -> { key: "auxclick", value: cb' (Cb (const (value $> true))) })) eventValue)
...
instance Attr anything OnAuxclick (Event.Event Unit -> Event.Event  (Effect Boolean) ) where
  attr OnAuxclick eventValue = unsafeAttribute (map (map (
    \value -> { key: "auxclick", value: cb' (Cb (const value)) })) eventValue) 
...
instance Attr everything OnAuxclick (Event.Event Unit -> Event.Event  Unit ) where
  attr OnAuxclick eventValue = unsafeAttribute (map (map (
    \_ -> { key: "auxclick", value: unset' })) eventValue)
