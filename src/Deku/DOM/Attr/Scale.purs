...
instance Attr FeDisplacementMap_ Scale (Event.Event Unit -> Event.Event  String ) where
  attr Scale eventValue = unsafeAttribute (map (map (
    \value -> { key: "scale", value: prop' value })) eventValue)
...
instance Attr everything Scale (Event.Event Unit -> Event.Event  Unit ) where
  attr Scale eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "scale", value: unset' })) eventValue)
