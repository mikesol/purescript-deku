...
instance Attr anything OnReset (Zora Unit) where
  attr OnReset value = unsafeAttribute { key: "reset", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnReset (Zora Boolean) where
  attr OnReset value = unsafeAttribute { key: "reset", value: cb' (Cb (const (runImpure value))) }