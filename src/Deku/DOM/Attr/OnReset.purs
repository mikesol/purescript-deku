...
instance Attr anything OnReset (Event.Event Unit -> Event.Event  Cb ) where
  attr OnReset eventValue = unsafeAttribute (map (map (
    \value -> { key: "reset", value: cb' value })) eventValue)
...
instance Attr anything OnReset (Event.Event Unit -> Event.Event  (Effect Unit) ) where
  attr OnReset eventValue = unsafeAttribute (map (map (
    \value -> { key: "reset", value: cb' (Cb (const (value $> true))) })) eventValue)
...
instance Attr anything OnReset (Event.Event Unit -> Event.Event  (Effect Boolean) ) where
  attr OnReset eventValue = unsafeAttribute (map (map (
    \value -> { key: "reset", value: cb' (Cb (const value)) })) eventValue) 
...
instance Attr everything OnReset (Event.Event Unit -> Event.Event  Unit ) where
  attr OnReset eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "reset", value: unset' })) eventValue)
