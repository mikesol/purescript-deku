...
instance Attr anything OnResize (Effect Unit) where
  attr OnResize value = unsafeAttribute { key: "resize", value: cb' (Cb (const (value $> true))) }
instance Attr anything OnResize (Effect Boolean) where
  attr OnResize value = unsafeAttribute { key: "resize", value: cb' (Cb (const value)) }
instance Attr anything OnResize (Zora Unit) where
  attr OnResize value = unsafeAttribute { key: "resize", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnResize (Zora Boolean) where
  attr OnResize value = unsafeAttribute { key: "resize", value: cb' (Cb (const (runImpure value))) }