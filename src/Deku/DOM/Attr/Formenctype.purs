...
instance Attr Button_ Formenctype (Event.Event Unit -> Event.Event  String ) where
  attr Formenctype eventValue = unsafeAttribute (map (map (
    \value -> { key: "formenctype", value: prop' value })) eventValue)
...
instance Attr Input_ Formenctype  String  where
  attr Formenctype value = unsafeAttribute (  
    { key: "formenctype", value: prop' value  } <$ _)
instance Attr Input_ Formenctype (Event.Event Unit -> Event.Event  String ) where
  attr Formenctype eventValue = unsafeAttribute (map (map (
    \value -> { key: "formenctype", value: prop' value })) eventValue)
...
instance Attr everything Formenctype (Event.Event Unit -> Event.Event  Unit ) where
  attr Formenctype eventValue = unsafeAttribute (map (map (
    \_ -> { key: "formenctype", value: unset' })) eventValue)
