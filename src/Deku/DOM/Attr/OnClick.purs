...
instance Attr anything OnClick (Event.Event Unit -> Event.Event Cb) where
  attr OnClick eventValue = unsafeAttribute (map (map (
    \value -> { key: "click", value: cb' value })) eventValue)
...
instance Attr anything OnClick (Event.Event Unit -> Event.Event (Effect Unit)) where
  attr OnClick eventValue = unsafeAttribute (map (map (
    \value -> { key: "click", value: cb' (Cb (const (value $> true))) })) eventValue)
...
instance Attr anything OnClick (Event.Event Unit -> Event.Event (Effect Boolean)) where
  attr OnClick eventValue = unsafeAttribute (map (map (
    \value -> { key: "click", value: cb' (Cb (const value)) })) eventValue)
...
instance Attr everything OnClick (Event.Event Unit -> Event.Event Unit) where
  attr OnClick eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "click", value: unset' })) eventValue)
