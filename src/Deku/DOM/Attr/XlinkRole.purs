...
instance Attr AnimateTransform_ XlinkRole (Event.Event Unit -> Event.Event  String ) where
  attr XlinkRole eventValue = unsafeAttribute (map (map (
    \value -> { key: "xlink:role", value: prop' value })) eventValue)
...
instance Attr FeImage_ XlinkRole  String  where
  attr XlinkRole value = unsafeAttribute (  
    { key: "xlink:role", value: prop' value  } <$ _)
instance Attr FeImage_ XlinkRole (Event.Event Unit -> Event.Event  String ) where
  attr XlinkRole eventValue = unsafeAttribute (map (map (
    \value -> { key: "xlink:role", value: prop' value })) eventValue)
...
instance Attr Filter_ XlinkRole  String  where
  attr XlinkRole value = unsafeAttribute (  
    { key: "xlink:role", value: prop' value  } <$ _)
instance Attr Filter_ XlinkRole (Event.Event Unit -> Event.Event  String ) where
  attr XlinkRole eventValue = unsafeAttribute (map (map (
    \value -> { key: "xlink:role", value: prop' value })) eventValue)
...
instance Attr Image_ XlinkRole  String  where
  attr XlinkRole value = unsafeAttribute (  
    { key: "xlink:role", value: prop' value  } <$ _)
instance Attr Image_ XlinkRole (Event.Event Unit -> Event.Event  String ) where
  attr XlinkRole eventValue = unsafeAttribute (map (map (
    \value -> { key: "xlink:role", value: prop' value })) eventValue)
...
instance Attr Mpath_ XlinkRole  String  where
  attr XlinkRole value = unsafeAttribute (  
    { key: "xlink:role", value: prop' value  } <$ _)
instance Attr Mpath_ XlinkRole (Event.Event Unit -> Event.Event  String ) where
  attr XlinkRole eventValue = unsafeAttribute (map (map (
    \value -> { key: "xlink:role", value: prop' value })) eventValue)
...
instance Attr everything XlinkRole (Event.Event Unit -> Event.Event  Unit ) where
  attr XlinkRole eventValue = unsafeAttribute (map (map (
    \_ -> { key: "xlink:role", value: unset' })) eventValue)
