...
instance Attr Img_ Intrinsicsize (Event.Event Unit -> Event.Event  String ) where
  attr Intrinsicsize eventValue = unsafeAttribute (map (map (
    \value -> { key: "intrinsicsize", value: prop' value })) eventValue)
...
instance Attr everything Intrinsicsize (Event.Event Unit -> Event.Event  Unit ) where
  attr Intrinsicsize eventValue = unsafeAttribute (map (map (
    \_ -> { key: "intrinsicsize", value: unset' })) eventValue)
