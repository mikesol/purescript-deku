...
instance Attr Filter_ FilterRes (Event.Event Unit -> Event.Event  String ) where
  attr FilterRes eventValue = unsafeAttribute (map (map (
    \value -> { key: "filterRes", value: prop' value })) eventValue)
...
instance Attr everything FilterRes (Event.Event Unit -> Event.Event  Unit ) where
  attr FilterRes eventValue = unsafeAttribute (map (map (
    \_ -> { key: "filterRes", value: unset' })) eventValue)
