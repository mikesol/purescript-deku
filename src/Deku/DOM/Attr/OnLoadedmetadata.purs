...
instance Attr anything OnLoadedmetadata (Zora Unit) where
  attr OnLoadedmetadata value = unsafeAttribute { key: "loadedmetadata", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnLoadedmetadata (Zora Boolean) where
  attr OnLoadedmetadata value = unsafeAttribute { key: "loadedmetadata", value: cb' (Cb (const (runImpure value))) }