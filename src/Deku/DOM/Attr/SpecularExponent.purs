...
instance Attr FeSpecularLighting_ SpecularExponent (Event.Event Unit -> Event.Event  String ) where
  attr SpecularExponent eventValue = unsafeAttribute (map (map ( \value -> { key: "specularExponent", value: prop' value })) eventValue)
...
instance Attr FeSpotLight_ SpecularExponent  String  where
  attr SpecularExponent value = unsafeAttribute (  
    { key: "specularExponent", value: prop' value  } <$ _)
instance Attr FeSpotLight_ SpecularExponent (Event.Event Unit -> Event.Event  String ) where
  attr SpecularExponent eventValue = unsafeAttribute (map (map ( \value -> { key: "specularExponent", value: prop' value })) eventValue)
...
instance Attr everything SpecularExponent (Event.Event Unit -> Event.Event  Unit ) where
  attr SpecularExponent eventValue = unsafeAttribute (map (map ( \_ -> { key: "specularExponent", value: unset' })) eventValue)
