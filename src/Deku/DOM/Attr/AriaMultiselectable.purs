...
instance Attr Circle_ AriaMultiselectable (Event.Event Unit -> Event.Event  String ) where
  attr AriaMultiselectable eventValue = unsafeAttribute (map (map ( \value -> { key: "aria-multiselectable", value: prop' value })) eventValue)
...
instance Attr Ellipse_ AriaMultiselectable  String  where
  attr AriaMultiselectable value = unsafeAttribute (  
    { key: "aria-multiselectable", value: prop' value  } <$ _)
instance Attr Ellipse_ AriaMultiselectable (Event.Event Unit -> Event.Event  String ) where
  attr AriaMultiselectable eventValue = unsafeAttribute (map (map ( \value -> { key: "aria-multiselectable", value: prop' value })) eventValue)
...
instance Attr ForeignObject_ AriaMultiselectable  String  where
  attr AriaMultiselectable value = unsafeAttribute (  
    { key: "aria-multiselectable", value: prop' value  } <$ _)
instance Attr ForeignObject_ AriaMultiselectable (Event.Event Unit -> Event.Event  String ) where
  attr AriaMultiselectable eventValue = unsafeAttribute (map (map ( \value -> { key: "aria-multiselectable", value: prop' value })) eventValue)
...
instance Attr G_ AriaMultiselectable  String  where
  attr AriaMultiselectable value = unsafeAttribute (  
    { key: "aria-multiselectable", value: prop' value  } <$ _)
instance Attr G_ AriaMultiselectable (Event.Event Unit -> Event.Event  String ) where
  attr AriaMultiselectable eventValue = unsafeAttribute (map (map ( \value -> { key: "aria-multiselectable", value: prop' value })) eventValue)
...
instance Attr Line_ AriaMultiselectable  String  where
  attr AriaMultiselectable value = unsafeAttribute (  
    { key: "aria-multiselectable", value: prop' value  } <$ _)
instance Attr Line_ AriaMultiselectable (Event.Event Unit -> Event.Event  String ) where
  attr AriaMultiselectable eventValue = unsafeAttribute (map (map ( \value -> { key: "aria-multiselectable", value: prop' value })) eventValue)
...
instance Attr Marker_ AriaMultiselectable  String  where
  attr AriaMultiselectable value = unsafeAttribute (  
    { key: "aria-multiselectable", value: prop' value  } <$ _)
instance Attr Marker_ AriaMultiselectable (Event.Event Unit -> Event.Event  String ) where
  attr AriaMultiselectable eventValue = unsafeAttribute (map (map ( \value -> { key: "aria-multiselectable", value: prop' value })) eventValue)
...
instance Attr Path_ AriaMultiselectable  String  where
  attr AriaMultiselectable value = unsafeAttribute (  
    { key: "aria-multiselectable", value: prop' value  } <$ _)
instance Attr Path_ AriaMultiselectable (Event.Event Unit -> Event.Event  String ) where
  attr AriaMultiselectable eventValue = unsafeAttribute (map (map ( \value -> { key: "aria-multiselectable", value: prop' value })) eventValue)
...
instance Attr Polygon_ AriaMultiselectable  String  where
  attr AriaMultiselectable value = unsafeAttribute (  
    { key: "aria-multiselectable", value: prop' value  } <$ _)
instance Attr Polygon_ AriaMultiselectable (Event.Event Unit -> Event.Event  String ) where
  attr AriaMultiselectable eventValue = unsafeAttribute (map (map ( \value -> { key: "aria-multiselectable", value: prop' value })) eventValue)
...
instance Attr Polyline_ AriaMultiselectable  String  where
  attr AriaMultiselectable value = unsafeAttribute (  
    { key: "aria-multiselectable", value: prop' value  } <$ _)
instance Attr Polyline_ AriaMultiselectable (Event.Event Unit -> Event.Event  String ) where
  attr AriaMultiselectable eventValue = unsafeAttribute (map (map ( \value -> { key: "aria-multiselectable", value: prop' value })) eventValue)
...
instance Attr Rect_ AriaMultiselectable  String  where
  attr AriaMultiselectable value = unsafeAttribute (  
    { key: "aria-multiselectable", value: prop' value  } <$ _)
instance Attr Rect_ AriaMultiselectable (Event.Event Unit -> Event.Event  String ) where
  attr AriaMultiselectable eventValue = unsafeAttribute (map (map ( \value -> { key: "aria-multiselectable", value: prop' value })) eventValue)
...
instance Attr Svg_ AriaMultiselectable  String  where
  attr AriaMultiselectable value = unsafeAttribute (  
    { key: "aria-multiselectable", value: prop' value  } <$ _)
instance Attr Svg_ AriaMultiselectable (Event.Event Unit -> Event.Event  String ) where
  attr AriaMultiselectable eventValue = unsafeAttribute (map (map ( \value -> { key: "aria-multiselectable", value: prop' value })) eventValue)
...
instance Attr Symbol_ AriaMultiselectable  String  where
  attr AriaMultiselectable value = unsafeAttribute (  
    { key: "aria-multiselectable", value: prop' value  } <$ _)
instance Attr Symbol_ AriaMultiselectable (Event.Event Unit -> Event.Event  String ) where
  attr AriaMultiselectable eventValue = unsafeAttribute (map (map ( \value -> { key: "aria-multiselectable", value: prop' value })) eventValue)
...
instance Attr Text_ AriaMultiselectable  String  where
  attr AriaMultiselectable value = unsafeAttribute (  
    { key: "aria-multiselectable", value: prop' value  } <$ _)
instance Attr Text_ AriaMultiselectable (Event.Event Unit -> Event.Event  String ) where
  attr AriaMultiselectable eventValue = unsafeAttribute (map (map ( \value -> { key: "aria-multiselectable", value: prop' value })) eventValue)
...
instance Attr TextPath_ AriaMultiselectable  String  where
  attr AriaMultiselectable value = unsafeAttribute (  
    { key: "aria-multiselectable", value: prop' value  } <$ _)
instance Attr TextPath_ AriaMultiselectable (Event.Event Unit -> Event.Event  String ) where
  attr AriaMultiselectable eventValue = unsafeAttribute (map (map ( \value -> { key: "aria-multiselectable", value: prop' value })) eventValue)
...
instance Attr Tspan_ AriaMultiselectable  String  where
  attr AriaMultiselectable value = unsafeAttribute (  
    { key: "aria-multiselectable", value: prop' value  } <$ _)
instance Attr Tspan_ AriaMultiselectable (Event.Event Unit -> Event.Event  String ) where
  attr AriaMultiselectable eventValue = unsafeAttribute (map (map ( \value -> { key: "aria-multiselectable", value: prop' value })) eventValue)
...
instance Attr Use_ AriaMultiselectable  String  where
  attr AriaMultiselectable value = unsafeAttribute (  
    { key: "aria-multiselectable", value: prop' value  } <$ _)
instance Attr Use_ AriaMultiselectable (Event.Event Unit -> Event.Event  String ) where
  attr AriaMultiselectable eventValue = unsafeAttribute (map (map ( \value -> { key: "aria-multiselectable", value: prop' value })) eventValue)
...
instance Attr View_ AriaMultiselectable  String  where
  attr AriaMultiselectable value = unsafeAttribute (  
    { key: "aria-multiselectable", value: prop' value  } <$ _)
instance Attr View_ AriaMultiselectable (Event.Event Unit -> Event.Event  String ) where
  attr AriaMultiselectable eventValue = unsafeAttribute (map (map ( \value -> { key: "aria-multiselectable", value: prop' value })) eventValue)
...
instance Attr everything AriaMultiselectable (Event.Event Unit -> Event.Event  Unit ) where
  attr AriaMultiselectable eventValue = unsafeAttribute (map (map ( \_ -> { key: "aria-multiselectable", value: unset' })) eventValue)
