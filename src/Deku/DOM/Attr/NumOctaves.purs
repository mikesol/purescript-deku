...
instance Attr FeTurbulence_ NumOctaves (Event.Event Unit -> Event.Event  String ) where
  attr NumOctaves eventValue = unsafeAttribute (map (map (
    \value -> { key: "numOctaves", value: prop' value })) eventValue)
...
instance Attr everything NumOctaves (Event.Event Unit -> Event.Event  Unit ) where
  attr NumOctaves eventValue = unsafeAttribute (map (map (
    \_ -> { key: "numOctaves", value: unset' })) eventValue)
