...
instance Attr Ellipse_ Ry (Event.Event Unit -> Event.Event  String ) where
  attr Ry eventValue = unsafeAttribute (map (map ( \value ->
    { key: "ry", value: prop' value })) eventValue)
...
instance Attr Rect_ Ry  String  where
  attr Ry value = unsafeAttribute (  { key: "ry", value: prop' value  } <$ _)
instance Attr Rect_ Ry (Event.Event Unit -> Event.Event  String ) where
  attr Ry eventValue = unsafeAttribute (map (map ( \value ->
    { key: "ry", value: prop' value })) eventValue)
...
instance Attr everything Ry (Event.Event Unit -> Event.Event  Unit ) where
  attr Ry eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "ry", value: unset' })) eventValue)
