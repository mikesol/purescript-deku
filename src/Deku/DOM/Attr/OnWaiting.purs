...
instance Attr anything OnWaiting (Zora Unit) where
  attr OnWaiting value = unsafeAttribute { key: "waiting", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnWaiting (Zora Boolean) where
  attr OnWaiting value = unsafeAttribute { key: "waiting", value: cb' (Cb (const (runImpure value))) }