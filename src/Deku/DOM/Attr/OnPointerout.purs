...
instance Attr anything OnPointerout (Effect Unit) where
  attr OnPointerout value = unsafeAttribute { key: "pointerout", value: cb' (Cb (const (value $> true))) }
instance Attr anything OnPointerout (Effect Boolean) where
  attr OnPointerout value = unsafeAttribute { key: "pointerout", value: cb' (Cb (const value)) }
instance Attr anything OnPointerout (Zora Unit) where
  attr OnPointerout value = unsafeAttribute { key: "pointerout", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnPointerout (Zora Boolean) where
  attr OnPointerout value = unsafeAttribute { key: "pointerout", value: cb' (Cb (const (runImpure value))) }