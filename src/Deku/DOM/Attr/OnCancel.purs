...
instance Attr anything OnCancel (Zora Unit) where
  attr OnCancel value = unsafeAttribute { key: "cancel", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnCancel (Zora Boolean) where
  attr OnCancel value = unsafeAttribute { key: "cancel", value: cb' (Cb (const (runImpure value))) }