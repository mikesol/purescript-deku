...
instance Attr anything OnEnded (Event.Event Unit -> Event.Event  Cb ) where
  attr OnEnded eventValue = unsafeAttribute (map (map (
    \value -> { key: "ended", value: cb' value })) eventValue)
...
instance Attr anything OnEnded (Event.Event Unit -> Event.Event  (Effect Unit) ) where
  attr OnEnded eventValue = unsafeAttribute (map (map (
    \value -> { key: "ended", value: cb' (Cb (const (value $> true))) })) eventValue)
...
instance Attr anything OnEnded (Event.Event Unit -> Event.Event  (Effect Boolean) ) where
  attr OnEnded eventValue = unsafeAttribute (map (map (
    \value -> { key: "ended", value: cb' (Cb (const value)) })) eventValue) 
...
instance Attr everything OnEnded (Event.Event Unit -> Event.Event  Unit ) where
  attr OnEnded eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "ended", value: unset' })) eventValue)
