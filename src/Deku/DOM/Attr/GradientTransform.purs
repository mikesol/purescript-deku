...
instance Attr LinearGradient_ GradientTransform (Event.Event Unit -> Event.Event  String ) where
  attr GradientTransform eventValue = unsafeAttribute (map (map ( \value -> { key: "gradientTransform", value: prop' value })) eventValue)
...
instance Attr RadialGradient_ GradientTransform  String  where
  attr GradientTransform value = unsafeAttribute (  
    { key: "gradientTransform", value: prop' value  } <$ _)
instance Attr RadialGradient_ GradientTransform (Event.Event Unit -> Event.Event  String ) where
  attr GradientTransform eventValue = unsafeAttribute (map (map ( \value -> { key: "gradientTransform", value: prop' value })) eventValue)
...
instance Attr everything GradientTransform (Event.Event Unit -> Event.Event  Unit ) where
  attr GradientTransform eventValue = unsafeAttribute (map (map ( \_ -> { key: "gradientTransform", value: unset' })) eventValue)
