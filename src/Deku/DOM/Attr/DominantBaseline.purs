...
instance Attr FeBlend_ DominantBaseline (Event.Event Unit -> Event.Event  String ) where
  attr DominantBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "dominant-baseline", value: prop' value })) eventValue)
...
instance Attr FeColorMatrix_ DominantBaseline  String  where
  attr DominantBaseline value = unsafeAttribute (  
    { key: "dominant-baseline", value: prop' value  } <$ _)
instance Attr FeColorMatrix_ DominantBaseline (Event.Event Unit -> Event.Event  String ) where
  attr DominantBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "dominant-baseline", value: prop' value })) eventValue)
...
instance Attr FeComponentTransfer_ DominantBaseline  String  where
  attr DominantBaseline value = unsafeAttribute (  
    { key: "dominant-baseline", value: prop' value  } <$ _)
instance Attr FeComponentTransfer_ DominantBaseline (Event.Event Unit -> Event.Event  String ) where
  attr DominantBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "dominant-baseline", value: prop' value })) eventValue)
...
instance Attr FeComposite_ DominantBaseline  String  where
  attr DominantBaseline value = unsafeAttribute (  
    { key: "dominant-baseline", value: prop' value  } <$ _)
instance Attr FeComposite_ DominantBaseline (Event.Event Unit -> Event.Event  String ) where
  attr DominantBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "dominant-baseline", value: prop' value })) eventValue)
...
instance Attr FeConvolveMatrix_ DominantBaseline  String  where
  attr DominantBaseline value = unsafeAttribute (  
    { key: "dominant-baseline", value: prop' value  } <$ _)
instance Attr FeConvolveMatrix_ DominantBaseline (Event.Event Unit -> Event.Event  String ) where
  attr DominantBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "dominant-baseline", value: prop' value })) eventValue)
...
instance Attr FeDiffuseLighting_ DominantBaseline  String  where
  attr DominantBaseline value = unsafeAttribute (  
    { key: "dominant-baseline", value: prop' value  } <$ _)
instance Attr FeDiffuseLighting_ DominantBaseline (Event.Event Unit -> Event.Event  String ) where
  attr DominantBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "dominant-baseline", value: prop' value })) eventValue)
...
instance Attr FeDisplacementMap_ DominantBaseline  String  where
  attr DominantBaseline value = unsafeAttribute (  
    { key: "dominant-baseline", value: prop' value  } <$ _)
instance Attr FeDisplacementMap_ DominantBaseline (Event.Event Unit -> Event.Event  String ) where
  attr DominantBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "dominant-baseline", value: prop' value })) eventValue)
...
instance Attr FeFlood_ DominantBaseline  String  where
  attr DominantBaseline value = unsafeAttribute (  
    { key: "dominant-baseline", value: prop' value  } <$ _)
instance Attr FeFlood_ DominantBaseline (Event.Event Unit -> Event.Event  String ) where
  attr DominantBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "dominant-baseline", value: prop' value })) eventValue)
...
instance Attr FeGaussianBlur_ DominantBaseline  String  where
  attr DominantBaseline value = unsafeAttribute (  
    { key: "dominant-baseline", value: prop' value  } <$ _)
instance Attr FeGaussianBlur_ DominantBaseline (Event.Event Unit -> Event.Event  String ) where
  attr DominantBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "dominant-baseline", value: prop' value })) eventValue)
...
instance Attr FeImage_ DominantBaseline  String  where
  attr DominantBaseline value = unsafeAttribute (  
    { key: "dominant-baseline", value: prop' value  } <$ _)
instance Attr FeImage_ DominantBaseline (Event.Event Unit -> Event.Event  String ) where
  attr DominantBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "dominant-baseline", value: prop' value })) eventValue)
...
instance Attr FeMerge_ DominantBaseline  String  where
  attr DominantBaseline value = unsafeAttribute (  
    { key: "dominant-baseline", value: prop' value  } <$ _)
instance Attr FeMerge_ DominantBaseline (Event.Event Unit -> Event.Event  String ) where
  attr DominantBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "dominant-baseline", value: prop' value })) eventValue)
...
instance Attr FeMorphology_ DominantBaseline  String  where
  attr DominantBaseline value = unsafeAttribute (  
    { key: "dominant-baseline", value: prop' value  } <$ _)
instance Attr FeMorphology_ DominantBaseline (Event.Event Unit -> Event.Event  String ) where
  attr DominantBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "dominant-baseline", value: prop' value })) eventValue)
...
instance Attr FeOffset_ DominantBaseline  String  where
  attr DominantBaseline value = unsafeAttribute (  
    { key: "dominant-baseline", value: prop' value  } <$ _)
instance Attr FeOffset_ DominantBaseline (Event.Event Unit -> Event.Event  String ) where
  attr DominantBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "dominant-baseline", value: prop' value })) eventValue)
...
instance Attr FeSpecularLighting_ DominantBaseline  String  where
  attr DominantBaseline value = unsafeAttribute (  
    { key: "dominant-baseline", value: prop' value  } <$ _)
instance Attr FeSpecularLighting_ DominantBaseline (Event.Event Unit -> Event.Event  String ) where
  attr DominantBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "dominant-baseline", value: prop' value })) eventValue)
...
instance Attr FeTile_ DominantBaseline  String  where
  attr DominantBaseline value = unsafeAttribute (  
    { key: "dominant-baseline", value: prop' value  } <$ _)
instance Attr FeTile_ DominantBaseline (Event.Event Unit -> Event.Event  String ) where
  attr DominantBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "dominant-baseline", value: prop' value })) eventValue)
...
instance Attr FeTurbulence_ DominantBaseline  String  where
  attr DominantBaseline value = unsafeAttribute (  
    { key: "dominant-baseline", value: prop' value  } <$ _)
instance Attr FeTurbulence_ DominantBaseline (Event.Event Unit -> Event.Event  String ) where
  attr DominantBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "dominant-baseline", value: prop' value })) eventValue)
...
instance Attr Filter_ DominantBaseline  String  where
  attr DominantBaseline value = unsafeAttribute (  
    { key: "dominant-baseline", value: prop' value  } <$ _)
instance Attr Filter_ DominantBaseline (Event.Event Unit -> Event.Event  String ) where
  attr DominantBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "dominant-baseline", value: prop' value })) eventValue)
...
instance Attr Image_ DominantBaseline  String  where
  attr DominantBaseline value = unsafeAttribute (  
    { key: "dominant-baseline", value: prop' value  } <$ _)
instance Attr Image_ DominantBaseline (Event.Event Unit -> Event.Event  String ) where
  attr DominantBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "dominant-baseline", value: prop' value })) eventValue)
...
instance Attr Switch_ DominantBaseline  String  where
  attr DominantBaseline value = unsafeAttribute (  
    { key: "dominant-baseline", value: prop' value  } <$ _)
instance Attr Switch_ DominantBaseline (Event.Event Unit -> Event.Event  String ) where
  attr DominantBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "dominant-baseline", value: prop' value })) eventValue)
...
instance Attr Text_ DominantBaseline  String  where
  attr DominantBaseline value = unsafeAttribute (  
    { key: "dominant-baseline", value: prop' value  } <$ _)
instance Attr Text_ DominantBaseline (Event.Event Unit -> Event.Event  String ) where
  attr DominantBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "dominant-baseline", value: prop' value })) eventValue)
...
instance Attr Tspan_ DominantBaseline  String  where
  attr DominantBaseline value = unsafeAttribute (  
    { key: "dominant-baseline", value: prop' value  } <$ _)
instance Attr Tspan_ DominantBaseline (Event.Event Unit -> Event.Event  String ) where
  attr DominantBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "dominant-baseline", value: prop' value })) eventValue)
...
instance Attr everything DominantBaseline (Event.Event Unit -> Event.Event  Unit ) where
  attr DominantBaseline eventValue = unsafeAttribute (map (map ( \_ -> { key: "dominant-baseline", value: unset' })) eventValue)
