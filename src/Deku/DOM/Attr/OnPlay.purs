...
instance Attr anything OnPlay (Event.Event Unit -> Event.Event  Cb ) where
  attr OnPlay eventValue = unsafeAttribute (map (map (
    \value -> { key: "play", value: cb' value })) eventValue)
...
instance Attr anything OnPlay (Event.Event Unit -> Event.Event  (Effect Unit) ) where
  attr OnPlay eventValue = unsafeAttribute (map (map (
    \value -> { key: "play", value: cb' (Cb (const (value $> true))) })) eventValue)
...
instance Attr anything OnPlay (Event.Event Unit -> Event.Event  (Effect Boolean) ) where
  attr OnPlay eventValue = unsafeAttribute (map (map (
    \value -> { key: "play", value: cb' (Cb (const value)) })) eventValue) 
...
instance Attr everything OnPlay (Event.Event Unit -> Event.Event  Unit ) where
  attr OnPlay eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "play", value: unset' })) eventValue)
