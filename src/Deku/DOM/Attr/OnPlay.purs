...
instance Attr anything OnPlay (Zora Unit) where
  attr OnPlay value = unsafeAttribute { key: "play", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnPlay (Zora Boolean) where
  attr OnPlay value = unsafeAttribute { key: "play", value: cb' (Cb (const (runImpure value))) }