...
instance Attr anything OnPointerdown (Zora Unit) where
  attr OnPointerdown value = unsafeAttribute { key: "pointerdown", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnPointerdown (Zora Boolean) where
  attr OnPointerdown value = unsafeAttribute { key: "pointerdown", value: cb' (Cb (const (runImpure value))) }