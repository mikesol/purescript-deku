...
instance Attr Filter_ FilterUnits (Event.Event Unit -> Event.Event  String ) where
  attr FilterUnits eventValue = unsafeAttribute (map (map (
    \value -> { key: "filterUnits", value: prop' value })) eventValue)
...
instance Attr everything FilterUnits (Event.Event Unit -> Event.Event  Unit ) where
  attr FilterUnits eventValue = unsafeAttribute (map (map (
    \_ -> { key: "filterUnits", value: unset' })) eventValue)
