...
instance Attr anything OnDragstart (Event.Event Unit -> Event.Event  Cb ) where
  attr OnDragstart eventValue = unsafeAttribute (map (map (
    \value -> { key: "dragstart", value: cb' value })) eventValue)
...
instance Attr anything OnDragstart (Event.Event Unit -> Event.Event  (Effect Unit) ) where
  attr OnDragstart eventValue = unsafeAttribute (map (map (
    \value -> { key: "dragstart", value: cb' (Cb (const (value $> true))) })) eventValue)
...
instance Attr anything OnDragstart (Event.Event Unit -> Event.Event  (Effect Boolean) ) where
  attr OnDragstart eventValue = unsafeAttribute (map (map (
    \value -> { key: "dragstart", value: cb' (Cb (const value)) })) eventValue) 
...
instance Attr everything OnDragstart (Event.Event Unit -> Event.Event  Unit ) where
  attr OnDragstart eventValue = unsafeAttribute (map (map (
    \_ -> { key: "dragstart", value: unset' })) eventValue)
