...
instance Attr anything OnDurationchange (Event.Event Unit -> Event.Event  Cb ) where
  attr OnDurationchange eventValue = unsafeAttribute (map (map ( \value -> { key: "durationchange", value: cb' value })) eventValue)
...
instance Attr anything OnDurationchange (Event.Event Unit -> Event.Event  (Effect Unit) ) where
  attr OnDurationchange eventValue = unsafeAttribute (map (map ( \value ->
      { key: "durationchange", value: cb' (Cb (const (value $> true))) })) eventValue)
...
instance Attr anything OnDurationchange (Event.Event Unit -> Event.Event  (Effect Boolean) ) where
  attr OnDurationchange eventValue = unsafeAttribute (map (map ( \value -> { key: "durationchange", value: cb' (Cb (const value)) })) eventValue) 
...
instance Attr everything OnDurationchange (Event.Event Unit -> Event.Event  Unit ) where
  attr OnDurationchange eventValue = unsafeAttribute (map (map ( \_ -> { key: "durationchange", value: unset' })) eventValue)
