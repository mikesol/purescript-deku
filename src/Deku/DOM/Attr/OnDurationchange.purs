...
instance Attr anything OnDurationchange (Zora Unit) where
  attr OnDurationchange value = unsafeAttribute { key: "durationchange", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnDurationchange (Zora Boolean) where
  attr OnDurationchange value = unsafeAttribute { key: "durationchange", value: cb' (Cb (const (runImpure value))) }