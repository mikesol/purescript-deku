...
instance Attr anything OnChange (Zora Unit) where
  attr OnChange value = unsafeAttribute { key: "change", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnChange (Zora Boolean) where
  attr OnChange value = unsafeAttribute { key: "change", value: cb' (Cb (const (runImpure value))) }