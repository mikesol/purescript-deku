...
instance Attr Animate_ From (Event.Event Unit -> Event.Event  String ) where
  attr From eventValue = unsafeAttribute (map (map ( \value ->
    { key: "from", value: prop' value })) eventValue)
...
instance Attr AnimateMotion_ From  String  where
  attr From value = unsafeAttribute (  
    { key: "from", value: prop' value  } <$ _)
instance Attr AnimateMotion_ From (Event.Event Unit -> Event.Event  String ) where
  attr From eventValue = unsafeAttribute (map (map ( \value ->
    { key: "from", value: prop' value })) eventValue)
...
instance Attr AnimateTransform_ From  String  where
  attr From value = unsafeAttribute (  
    { key: "from", value: prop' value  } <$ _)
instance Attr AnimateTransform_ From (Event.Event Unit -> Event.Event  String ) where
  attr From eventValue = unsafeAttribute (map (map ( \value ->
    { key: "from", value: prop' value })) eventValue)
...
instance Attr everything From (Event.Event Unit -> Event.Event  Unit ) where
  attr From eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "from", value: unset' })) eventValue)
