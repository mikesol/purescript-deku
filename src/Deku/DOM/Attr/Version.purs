...
instance Attr Svg_ Version (Event.Event Unit -> Event.Event  String ) where
  attr Version eventValue = unsafeAttribute (map (map (
    \value -> { key: "version", value: prop' value })) eventValue)
...
instance Attr everything Version (Event.Event Unit -> Event.Event  Unit ) where
  attr Version eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "version", value: unset' })) eventValue)
