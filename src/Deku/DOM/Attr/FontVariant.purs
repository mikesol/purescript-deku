...
instance Attr FeBlend_ FontVariant (Event.Event Unit -> Event.Event  String ) where
  attr FontVariant eventValue = unsafeAttribute (map (map (
    \value -> { key: "font-variant", value: prop' value })) eventValue)
...
instance Attr FeColorMatrix_ FontVariant  String  where
  attr FontVariant value = unsafeAttribute (  
    { key: "font-variant", value: prop' value  } <$ _)
instance Attr FeColorMatrix_ FontVariant (Event.Event Unit -> Event.Event  String ) where
  attr FontVariant eventValue = unsafeAttribute (map (map (
    \value -> { key: "font-variant", value: prop' value })) eventValue)
...
instance Attr FeComponentTransfer_ FontVariant  String  where
  attr FontVariant value = unsafeAttribute (  
    { key: "font-variant", value: prop' value  } <$ _)
instance Attr FeComponentTransfer_ FontVariant (Event.Event Unit -> Event.Event  String ) where
  attr FontVariant eventValue = unsafeAttribute (map (map (
    \value -> { key: "font-variant", value: prop' value })) eventValue)
...
instance Attr FeComposite_ FontVariant  String  where
  attr FontVariant value = unsafeAttribute (  
    { key: "font-variant", value: prop' value  } <$ _)
instance Attr FeComposite_ FontVariant (Event.Event Unit -> Event.Event  String ) where
  attr FontVariant eventValue = unsafeAttribute (map (map (
    \value -> { key: "font-variant", value: prop' value })) eventValue)
...
instance Attr FeConvolveMatrix_ FontVariant  String  where
  attr FontVariant value = unsafeAttribute (  
    { key: "font-variant", value: prop' value  } <$ _)
instance Attr FeConvolveMatrix_ FontVariant (Event.Event Unit -> Event.Event  String ) where
  attr FontVariant eventValue = unsafeAttribute (map (map (
    \value -> { key: "font-variant", value: prop' value })) eventValue)
...
instance Attr FeDiffuseLighting_ FontVariant  String  where
  attr FontVariant value = unsafeAttribute (  
    { key: "font-variant", value: prop' value  } <$ _)
instance Attr FeDiffuseLighting_ FontVariant (Event.Event Unit -> Event.Event  String ) where
  attr FontVariant eventValue = unsafeAttribute (map (map (
    \value -> { key: "font-variant", value: prop' value })) eventValue)
...
instance Attr FeDisplacementMap_ FontVariant  String  where
  attr FontVariant value = unsafeAttribute (  
    { key: "font-variant", value: prop' value  } <$ _)
instance Attr FeDisplacementMap_ FontVariant (Event.Event Unit -> Event.Event  String ) where
  attr FontVariant eventValue = unsafeAttribute (map (map (
    \value -> { key: "font-variant", value: prop' value })) eventValue)
...
instance Attr FeFlood_ FontVariant  String  where
  attr FontVariant value = unsafeAttribute (  
    { key: "font-variant", value: prop' value  } <$ _)
instance Attr FeFlood_ FontVariant (Event.Event Unit -> Event.Event  String ) where
  attr FontVariant eventValue = unsafeAttribute (map (map (
    \value -> { key: "font-variant", value: prop' value })) eventValue)
...
instance Attr FeGaussianBlur_ FontVariant  String  where
  attr FontVariant value = unsafeAttribute (  
    { key: "font-variant", value: prop' value  } <$ _)
instance Attr FeGaussianBlur_ FontVariant (Event.Event Unit -> Event.Event  String ) where
  attr FontVariant eventValue = unsafeAttribute (map (map (
    \value -> { key: "font-variant", value: prop' value })) eventValue)
...
instance Attr FeImage_ FontVariant  String  where
  attr FontVariant value = unsafeAttribute (  
    { key: "font-variant", value: prop' value  } <$ _)
instance Attr FeImage_ FontVariant (Event.Event Unit -> Event.Event  String ) where
  attr FontVariant eventValue = unsafeAttribute (map (map (
    \value -> { key: "font-variant", value: prop' value })) eventValue)
...
instance Attr FeMerge_ FontVariant  String  where
  attr FontVariant value = unsafeAttribute (  
    { key: "font-variant", value: prop' value  } <$ _)
instance Attr FeMerge_ FontVariant (Event.Event Unit -> Event.Event  String ) where
  attr FontVariant eventValue = unsafeAttribute (map (map (
    \value -> { key: "font-variant", value: prop' value })) eventValue)
...
instance Attr FeMorphology_ FontVariant  String  where
  attr FontVariant value = unsafeAttribute (  
    { key: "font-variant", value: prop' value  } <$ _)
instance Attr FeMorphology_ FontVariant (Event.Event Unit -> Event.Event  String ) where
  attr FontVariant eventValue = unsafeAttribute (map (map (
    \value -> { key: "font-variant", value: prop' value })) eventValue)
...
instance Attr FeOffset_ FontVariant  String  where
  attr FontVariant value = unsafeAttribute (  
    { key: "font-variant", value: prop' value  } <$ _)
instance Attr FeOffset_ FontVariant (Event.Event Unit -> Event.Event  String ) where
  attr FontVariant eventValue = unsafeAttribute (map (map (
    \value -> { key: "font-variant", value: prop' value })) eventValue)
...
instance Attr FeSpecularLighting_ FontVariant  String  where
  attr FontVariant value = unsafeAttribute (  
    { key: "font-variant", value: prop' value  } <$ _)
instance Attr FeSpecularLighting_ FontVariant (Event.Event Unit -> Event.Event  String ) where
  attr FontVariant eventValue = unsafeAttribute (map (map (
    \value -> { key: "font-variant", value: prop' value })) eventValue)
...
instance Attr FeTile_ FontVariant  String  where
  attr FontVariant value = unsafeAttribute (  
    { key: "font-variant", value: prop' value  } <$ _)
instance Attr FeTile_ FontVariant (Event.Event Unit -> Event.Event  String ) where
  attr FontVariant eventValue = unsafeAttribute (map (map (
    \value -> { key: "font-variant", value: prop' value })) eventValue)
...
instance Attr FeTurbulence_ FontVariant  String  where
  attr FontVariant value = unsafeAttribute (  
    { key: "font-variant", value: prop' value  } <$ _)
instance Attr FeTurbulence_ FontVariant (Event.Event Unit -> Event.Event  String ) where
  attr FontVariant eventValue = unsafeAttribute (map (map (
    \value -> { key: "font-variant", value: prop' value })) eventValue)
...
instance Attr Filter_ FontVariant  String  where
  attr FontVariant value = unsafeAttribute (  
    { key: "font-variant", value: prop' value  } <$ _)
instance Attr Filter_ FontVariant (Event.Event Unit -> Event.Event  String ) where
  attr FontVariant eventValue = unsafeAttribute (map (map (
    \value -> { key: "font-variant", value: prop' value })) eventValue)
...
instance Attr Image_ FontVariant  String  where
  attr FontVariant value = unsafeAttribute (  
    { key: "font-variant", value: prop' value  } <$ _)
instance Attr Image_ FontVariant (Event.Event Unit -> Event.Event  String ) where
  attr FontVariant eventValue = unsafeAttribute (map (map (
    \value -> { key: "font-variant", value: prop' value })) eventValue)
...
instance Attr Switch_ FontVariant  String  where
  attr FontVariant value = unsafeAttribute (  
    { key: "font-variant", value: prop' value  } <$ _)
instance Attr Switch_ FontVariant (Event.Event Unit -> Event.Event  String ) where
  attr FontVariant eventValue = unsafeAttribute (map (map (
    \value -> { key: "font-variant", value: prop' value })) eventValue)
...
instance Attr Text_ FontVariant  String  where
  attr FontVariant value = unsafeAttribute (  
    { key: "font-variant", value: prop' value  } <$ _)
instance Attr Text_ FontVariant (Event.Event Unit -> Event.Event  String ) where
  attr FontVariant eventValue = unsafeAttribute (map (map (
    \value -> { key: "font-variant", value: prop' value })) eventValue)
...
instance Attr Tspan_ FontVariant  String  where
  attr FontVariant value = unsafeAttribute (  
    { key: "font-variant", value: prop' value  } <$ _)
instance Attr Tspan_ FontVariant (Event.Event Unit -> Event.Event  String ) where
  attr FontVariant eventValue = unsafeAttribute (map (map (
    \value -> { key: "font-variant", value: prop' value })) eventValue)
...
instance Attr everything FontVariant (Event.Event Unit -> Event.Event  Unit ) where
  attr FontVariant eventValue = unsafeAttribute (map (map (
    \_ -> { key: "font-variant", value: unset' })) eventValue)
