...
instance Attr anything OnRatechange (Effect Unit) where
  attr OnRatechange value = unsafeAttribute { key: "ratechange", value: cb' (Cb (const (value $> true))) }
instance Attr anything OnRatechange (Effect Boolean) where
  attr OnRatechange value = unsafeAttribute { key: "ratechange", value: cb' (Cb (const value)) }
instance Attr anything OnRatechange (Zora Unit) where
  attr OnRatechange value = unsafeAttribute { key: "ratechange", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnRatechange (Zora Boolean) where
  attr OnRatechange value = unsafeAttribute { key: "ratechange", value: cb' (Cb (const (runImpure value))) }