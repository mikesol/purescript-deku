...
instance Attr AnimateTransform_ XlinkType (Event.Event Unit -> Event.Event  String ) where
  attr XlinkType eventValue = unsafeAttribute (map (map (
    \value -> { key: "xlink:type", value: prop' value })) eventValue)
...
instance Attr FeImage_ XlinkType  String  where
  attr XlinkType value = unsafeAttribute (  
    { key: "xlink:type", value: prop' value  } <$ _)
instance Attr FeImage_ XlinkType (Event.Event Unit -> Event.Event  String ) where
  attr XlinkType eventValue = unsafeAttribute (map (map (
    \value -> { key: "xlink:type", value: prop' value })) eventValue)
...
instance Attr Filter_ XlinkType  String  where
  attr XlinkType value = unsafeAttribute (  
    { key: "xlink:type", value: prop' value  } <$ _)
instance Attr Filter_ XlinkType (Event.Event Unit -> Event.Event  String ) where
  attr XlinkType eventValue = unsafeAttribute (map (map (
    \value -> { key: "xlink:type", value: prop' value })) eventValue)
...
instance Attr Image_ XlinkType  String  where
  attr XlinkType value = unsafeAttribute (  
    { key: "xlink:type", value: prop' value  } <$ _)
instance Attr Image_ XlinkType (Event.Event Unit -> Event.Event  String ) where
  attr XlinkType eventValue = unsafeAttribute (map (map (
    \value -> { key: "xlink:type", value: prop' value })) eventValue)
...
instance Attr Mpath_ XlinkType  String  where
  attr XlinkType value = unsafeAttribute (  
    { key: "xlink:type", value: prop' value  } <$ _)
instance Attr Mpath_ XlinkType (Event.Event Unit -> Event.Event  String ) where
  attr XlinkType eventValue = unsafeAttribute (map (map (
    \value -> { key: "xlink:type", value: prop' value })) eventValue)
...
instance Attr everything XlinkType (Event.Event Unit -> Event.Event  Unit ) where
  attr XlinkType eventValue = unsafeAttribute (map (map (
    \_ -> { key: "xlink:type", value: unset' })) eventValue)
