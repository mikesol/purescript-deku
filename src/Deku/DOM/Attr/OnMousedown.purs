...
instance Attr anything OnMousedown (Zora Unit) where
  attr OnMousedown value = unsafeAttribute { key: "mousedown", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnMousedown (Zora Boolean) where
  attr OnMousedown value = unsafeAttribute { key: "mousedown", value: cb' (Cb (const (runImpure value))) }