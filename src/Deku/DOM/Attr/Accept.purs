...
instance Attr Form_ Accept (Event.Event Unit -> Event.Event  String ) where
  attr Accept eventValue = unsafeAttribute (map (map (
    \value -> { key: "accept", value: prop' value })) eventValue)
...
instance Attr Input_ Accept  String  where
  attr Accept value = unsafeAttribute (  
    { key: "accept", value: prop' value  } <$ _)
instance Attr Input_ Accept (Event.Event Unit -> Event.Event  String ) where
  attr Accept eventValue = unsafeAttribute (map (map (
    \value -> { key: "accept", value: prop' value })) eventValue)
...
instance Attr everything Accept (Event.Event Unit -> Event.Event  Unit ) where
  attr Accept eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "accept", value: unset' })) eventValue)
