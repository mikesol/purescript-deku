...
instance Attr FeDropShadow_ Dx (Event.Event Unit -> Event.Event  String ) where
  attr Dx eventValue = unsafeAttribute (map (map ( \value ->
    { key: "dx", value: prop' value })) eventValue)
...
instance Attr FeOffset_ Dx  String  where
  attr Dx value = unsafeAttribute (  { key: "dx", value: prop' value  } <$ _)
instance Attr FeOffset_ Dx (Event.Event Unit -> Event.Event  String ) where
  attr Dx eventValue = unsafeAttribute (map (map ( \value ->
    { key: "dx", value: prop' value })) eventValue)
...
instance Attr Text_ Dx  String  where
  attr Dx value = unsafeAttribute (  { key: "dx", value: prop' value  } <$ _)
instance Attr Text_ Dx (Event.Event Unit -> Event.Event  String ) where
  attr Dx eventValue = unsafeAttribute (map (map ( \value ->
    { key: "dx", value: prop' value })) eventValue)
...
instance Attr Tspan_ Dx  String  where
  attr Dx value = unsafeAttribute (  { key: "dx", value: prop' value  } <$ _)
instance Attr Tspan_ Dx (Event.Event Unit -> Event.Event  String ) where
  attr Dx eventValue = unsafeAttribute (map (map ( \value ->
    { key: "dx", value: prop' value })) eventValue)
...
instance Attr everything Dx (Event.Event Unit -> Event.Event  Unit ) where
  attr Dx eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "dx", value: unset' })) eventValue)
