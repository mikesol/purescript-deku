...
instance Attr anything OnTouchstart   (Zora Unit) where
  attr OnTouchstart   value = unsafeAttribute { key: "touchstart  ", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnTouchstart   (Zora Boolean) where
  attr OnTouchstart   value = unsafeAttribute { key: "touchstart  ", value: cb' (Cb (const (runImpure value))) }