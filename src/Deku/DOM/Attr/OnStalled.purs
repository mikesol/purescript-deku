...
instance Attr anything OnStalled (Zora Unit) where
  attr OnStalled value = unsafeAttribute { key: "stalled", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnStalled (Zora Boolean) where
  attr OnStalled value = unsafeAttribute { key: "stalled", value: cb' (Cb (const (runImpure value))) }