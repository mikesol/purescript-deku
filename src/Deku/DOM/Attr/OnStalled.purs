...
instance Attr anything OnStalled (NonEmpty.NonEmpty Event.Event  Cb ) where
  attr OnStalled bothValues = unsafeAttribute $ Both (pure 
    { key: "stalled", value: cb' (NonEmpty.head bothValues) })
    (NonEmpty.tail bothValues <#> \value -> { key: "stalled", value: cb' value })
instance Attr anything OnStalled (Product.Product (ST.ST Global.Global) Event.Event  Cb ) where
  attr OnStalled (Product.Product bothValues) = unsafeAttribute $ Both (Tuple.fst bothValues <#> \value ->  
    { key: "stalled", value: cb' (value) })
    (Tuple.snd bothValues <#> \value -> { key: "stalled", value: cb' value })
...
instance Attr anything OnStalled (ST.ST Global.Global  Cb ) where
  attr OnStalled stValue = unsafeAttribute $ This $ stValue <#>
    \value -> { key: "stalled", value: cb' value }
...
instance Attr anything OnStalled (Product.Product (ST.ST Global.Global) Event.Event  (Effect Unit) ) where
  attr OnStalled (Product.Product bothValues) = unsafeAttribute $ Both (Tuple.fst bothValues <#> \value ->  
    { key: "stalled", value: cb' (Cb (const ((value) $> true))) })
...
instance Attr anything OnStalled (ST.ST Global.Global  (Effect Unit) ) where
  attr OnStalled stValue = unsafeAttribute $ This $ stValue <#>
    \value -> { key: "stalled", value: cb' (Cb (const (value $> true))) }
...
instance Attr anything OnStalled (Product.Product (ST.ST Global.Global) Event.Event  (Effect Boolean) ) where
  attr OnStalled (Product.Product bothValues) = unsafeAttribute $ Both (Tuple.fst bothValues <#> \value ->  
    { key: "stalled", value: cb' (Cb (const (value))) })
    ( Tuple.snd bothValues <#> \value ->
        { key: "stalled", value: cb' (Cb (const value)) }
...
instance Attr anything OnStalled (ST.ST Global.Global  (Effect Boolean) ) where
  attr OnStalled stValue = unsafeAttribute $ This $ stValue <#>
    \value -> { key: "stalled", value: cb' (Cb (const value)) }
...
instance Attr everything OnStalled (Product.Product (ST.ST Global.Global) Event.Event  Unit ) where
  attr OnStalled (Product.Product bothValues) = unsafeAttribute $ Both (Tuple.fst bothValues <#> \_ ->  
    { key: "stalled", value: unset' })
    (Tuple.snd bothValues <#> \_ -> { key: "stalled", value: unset' })
...
instance Attr everything OnStalled (ST.ST Global.Global  Unit ) where
  attr OnStalled stValue = unsafeAttribute $ This $ stValue <#>
    \_ -> { key: "stalled", value: unset' }
