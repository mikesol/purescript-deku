...
instance Attr FeBlend_ AlignmentBaseline (Event.Event Unit -> Event.Event  String ) where
  attr AlignmentBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "alignment-baseline", value: prop' value })) eventValue)
...
instance Attr FeColorMatrix_ AlignmentBaseline  String  where
  attr AlignmentBaseline value = unsafeAttribute (  
    { key: "alignment-baseline", value: prop' value  } <$ _)
instance Attr FeColorMatrix_ AlignmentBaseline (Event.Event Unit -> Event.Event  String ) where
  attr AlignmentBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "alignment-baseline", value: prop' value })) eventValue)
...
instance Attr FeComponentTransfer_ AlignmentBaseline  String  where
  attr AlignmentBaseline value = unsafeAttribute (  
    { key: "alignment-baseline", value: prop' value  } <$ _)
instance Attr FeComponentTransfer_ AlignmentBaseline (Event.Event Unit -> Event.Event  String ) where
  attr AlignmentBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "alignment-baseline", value: prop' value })) eventValue)
...
instance Attr FeComposite_ AlignmentBaseline  String  where
  attr AlignmentBaseline value = unsafeAttribute (  
    { key: "alignment-baseline", value: prop' value  } <$ _)
instance Attr FeComposite_ AlignmentBaseline (Event.Event Unit -> Event.Event  String ) where
  attr AlignmentBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "alignment-baseline", value: prop' value })) eventValue)
...
instance Attr FeConvolveMatrix_ AlignmentBaseline  String  where
  attr AlignmentBaseline value = unsafeAttribute (  
    { key: "alignment-baseline", value: prop' value  } <$ _)
instance Attr FeConvolveMatrix_ AlignmentBaseline (Event.Event Unit -> Event.Event  String ) where
  attr AlignmentBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "alignment-baseline", value: prop' value })) eventValue)
...
instance Attr FeDiffuseLighting_ AlignmentBaseline  String  where
  attr AlignmentBaseline value = unsafeAttribute (  
    { key: "alignment-baseline", value: prop' value  } <$ _)
instance Attr FeDiffuseLighting_ AlignmentBaseline (Event.Event Unit -> Event.Event  String ) where
  attr AlignmentBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "alignment-baseline", value: prop' value })) eventValue)
...
instance Attr FeDisplacementMap_ AlignmentBaseline  String  where
  attr AlignmentBaseline value = unsafeAttribute (  
    { key: "alignment-baseline", value: prop' value  } <$ _)
instance Attr FeDisplacementMap_ AlignmentBaseline (Event.Event Unit -> Event.Event  String ) where
  attr AlignmentBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "alignment-baseline", value: prop' value })) eventValue)
...
instance Attr FeFlood_ AlignmentBaseline  String  where
  attr AlignmentBaseline value = unsafeAttribute (  
    { key: "alignment-baseline", value: prop' value  } <$ _)
instance Attr FeFlood_ AlignmentBaseline (Event.Event Unit -> Event.Event  String ) where
  attr AlignmentBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "alignment-baseline", value: prop' value })) eventValue)
...
instance Attr FeGaussianBlur_ AlignmentBaseline  String  where
  attr AlignmentBaseline value = unsafeAttribute (  
    { key: "alignment-baseline", value: prop' value  } <$ _)
instance Attr FeGaussianBlur_ AlignmentBaseline (Event.Event Unit -> Event.Event  String ) where
  attr AlignmentBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "alignment-baseline", value: prop' value })) eventValue)
...
instance Attr FeImage_ AlignmentBaseline  String  where
  attr AlignmentBaseline value = unsafeAttribute (  
    { key: "alignment-baseline", value: prop' value  } <$ _)
instance Attr FeImage_ AlignmentBaseline (Event.Event Unit -> Event.Event  String ) where
  attr AlignmentBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "alignment-baseline", value: prop' value })) eventValue)
...
instance Attr FeMerge_ AlignmentBaseline  String  where
  attr AlignmentBaseline value = unsafeAttribute (  
    { key: "alignment-baseline", value: prop' value  } <$ _)
instance Attr FeMerge_ AlignmentBaseline (Event.Event Unit -> Event.Event  String ) where
  attr AlignmentBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "alignment-baseline", value: prop' value })) eventValue)
...
instance Attr FeMorphology_ AlignmentBaseline  String  where
  attr AlignmentBaseline value = unsafeAttribute (  
    { key: "alignment-baseline", value: prop' value  } <$ _)
instance Attr FeMorphology_ AlignmentBaseline (Event.Event Unit -> Event.Event  String ) where
  attr AlignmentBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "alignment-baseline", value: prop' value })) eventValue)
...
instance Attr FeOffset_ AlignmentBaseline  String  where
  attr AlignmentBaseline value = unsafeAttribute (  
    { key: "alignment-baseline", value: prop' value  } <$ _)
instance Attr FeOffset_ AlignmentBaseline (Event.Event Unit -> Event.Event  String ) where
  attr AlignmentBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "alignment-baseline", value: prop' value })) eventValue)
...
instance Attr FeSpecularLighting_ AlignmentBaseline  String  where
  attr AlignmentBaseline value = unsafeAttribute (  
    { key: "alignment-baseline", value: prop' value  } <$ _)
instance Attr FeSpecularLighting_ AlignmentBaseline (Event.Event Unit -> Event.Event  String ) where
  attr AlignmentBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "alignment-baseline", value: prop' value })) eventValue)
...
instance Attr FeTile_ AlignmentBaseline  String  where
  attr AlignmentBaseline value = unsafeAttribute (  
    { key: "alignment-baseline", value: prop' value  } <$ _)
instance Attr FeTile_ AlignmentBaseline (Event.Event Unit -> Event.Event  String ) where
  attr AlignmentBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "alignment-baseline", value: prop' value })) eventValue)
...
instance Attr FeTurbulence_ AlignmentBaseline  String  where
  attr AlignmentBaseline value = unsafeAttribute (  
    { key: "alignment-baseline", value: prop' value  } <$ _)
instance Attr FeTurbulence_ AlignmentBaseline (Event.Event Unit -> Event.Event  String ) where
  attr AlignmentBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "alignment-baseline", value: prop' value })) eventValue)
...
instance Attr Filter_ AlignmentBaseline  String  where
  attr AlignmentBaseline value = unsafeAttribute (  
    { key: "alignment-baseline", value: prop' value  } <$ _)
instance Attr Filter_ AlignmentBaseline (Event.Event Unit -> Event.Event  String ) where
  attr AlignmentBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "alignment-baseline", value: prop' value })) eventValue)
...
instance Attr Image_ AlignmentBaseline  String  where
  attr AlignmentBaseline value = unsafeAttribute (  
    { key: "alignment-baseline", value: prop' value  } <$ _)
instance Attr Image_ AlignmentBaseline (Event.Event Unit -> Event.Event  String ) where
  attr AlignmentBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "alignment-baseline", value: prop' value })) eventValue)
...
instance Attr Switch_ AlignmentBaseline  String  where
  attr AlignmentBaseline value = unsafeAttribute (  
    { key: "alignment-baseline", value: prop' value  } <$ _)
instance Attr Switch_ AlignmentBaseline (Event.Event Unit -> Event.Event  String ) where
  attr AlignmentBaseline eventValue = unsafeAttribute (map (map ( \value -> { key: "alignment-baseline", value: prop' value })) eventValue)
...
instance Attr everything AlignmentBaseline (Event.Event Unit -> Event.Event  Unit ) where
  attr AlignmentBaseline eventValue = unsafeAttribute (map (map ( \_ -> { key: "alignment-baseline", value: unset' })) eventValue)
