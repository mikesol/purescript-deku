...
instance Attr anything OnKeypress (Event.Event Unit -> Event.Event  Cb ) where
  attr OnKeypress eventValue = unsafeAttribute (map (map (
    \value -> { key: "keypress", value: cb' value })) eventValue)
...
instance Attr anything OnKeypress (Event.Event Unit -> Event.Event  (Effect Unit) ) where
  attr OnKeypress eventValue = unsafeAttribute (map (map (
    \value -> { key: "keypress", value: cb' (Cb (const (value $> true))) })) eventValue)
...
instance Attr anything OnKeypress (Event.Event Unit -> Event.Event  (Effect Boolean) ) where
  attr OnKeypress eventValue = unsafeAttribute (map (map (
    \value -> { key: "keypress", value: cb' (Cb (const value)) })) eventValue) 
...
instance Attr everything OnKeypress (Event.Event Unit -> Event.Event  Unit ) where
  attr OnKeypress eventValue = unsafeAttribute (map (map (
    \_ -> { key: "keypress", value: unset' })) eventValue)
