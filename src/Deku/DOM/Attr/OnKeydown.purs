...
instance Attr anything OnKeydown (Zora Unit) where
  attr OnKeydown value = unsafeAttribute { key: "keydown", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnKeydown (Zora Boolean) where
  attr OnKeydown value = unsafeAttribute { key: "keydown", value: cb' (Cb (const (runImpure value))) }