...
instance Attr anything OnTouchend   (Effect Unit) where
  attr OnTouchend   value = unsafeAttribute { key: "touchend  ", value: cb' (Cb (const (value $> true))) }
instance Attr anything OnTouchend   (Effect Boolean) where
  attr OnTouchend   value = unsafeAttribute { key: "touchend  ", value: cb' (Cb (const value)) }
instance Attr anything OnTouchend   (Zora Unit) where
  attr OnTouchend   value = unsafeAttribute { key: "touchend  ", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnTouchend   (Zora Boolean) where
  attr OnTouchend   value = unsafeAttribute { key: "touchend  ", value: cb' (Cb (const (runImpure value))) }