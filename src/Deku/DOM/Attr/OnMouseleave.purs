...
instance Attr anything OnMouseleave (Zora Unit) where
  attr OnMouseleave value = unsafeAttribute { key: "mouseleave", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnMouseleave (Zora Boolean) where
  attr OnMouseleave value = unsafeAttribute { key: "mouseleave", value: cb' (Cb (const (runImpure value))) }