...
instance Attr Track_ Srclang (Event.Event Unit -> Event.Event  String ) where
  attr Srclang eventValue = unsafeAttribute (map (map (
    \value -> { key: "srclang", value: prop' value })) eventValue)
...
instance Attr everything Srclang (Event.Event Unit -> Event.Event  Unit ) where
  attr Srclang eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "srclang", value: unset' })) eventValue)
