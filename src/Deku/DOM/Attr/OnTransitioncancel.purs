...
instance Attr anything OnTransitioncancel (Zora Unit) where
  attr OnTransitioncancel value = unsafeAttribute { key: "transitioncancel", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnTransitioncancel (Zora Boolean) where
  attr OnTransitioncancel value = unsafeAttribute { key: "transitioncancel", value: cb' (Cb (const (runImpure value))) }