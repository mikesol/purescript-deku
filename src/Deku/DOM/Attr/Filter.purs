...
instance Attr Circle_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr ClipPath_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr ClipPath_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr Defs_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr Defs_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr Ellipse_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr Ellipse_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr FeBlend_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr FeBlend_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr FeColorMatrix_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr FeColorMatrix_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr FeComponentTransfer_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr FeComponentTransfer_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr FeComposite_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr FeComposite_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr FeConvolveMatrix_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr FeConvolveMatrix_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr FeDiffuseLighting_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr FeDiffuseLighting_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr FeDisplacementMap_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr FeDisplacementMap_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr FeFlood_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr FeFlood_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr FeGaussianBlur_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr FeGaussianBlur_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr FeImage_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr FeImage_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr FeMerge_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr FeMerge_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr FeMorphology_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr FeMorphology_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr FeOffset_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr FeOffset_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr FeSpecularLighting_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr FeSpecularLighting_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr FeTile_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr FeTile_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr FeTurbulence_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr FeTurbulence_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr Filter_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr Filter_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr ForeignObject_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr ForeignObject_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr G_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr G_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr Image_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr Image_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr Line_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr Line_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr LinearGradient_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr LinearGradient_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr Marker_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr Marker_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr Mask_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr Mask_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr Path_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr Path_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr Pattern_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr Pattern_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr Polygon_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr Polygon_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr Polyline_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr Polyline_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr RadialGradient_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr RadialGradient_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr Rect_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr Rect_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr Svg_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr Svg_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr Switch_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr Switch_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr Symbol_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr Symbol_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr Text_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr Text_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr TextPath_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr TextPath_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr Tspan_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr Tspan_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr Use_ Filter  String  where
  attr Filter value = unsafeAttribute (  
    { key: "filter", value: prop' value  } <$ _)
instance Attr Use_ Filter (Event.Event Unit -> Event.Event  String ) where
  attr Filter eventValue = unsafeAttribute (map (map (
    \value -> { key: "filter", value: prop' value })) eventValue)
...
instance Attr everything Filter (Event.Event Unit -> Event.Event  Unit ) where
  attr Filter eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "filter", value: unset' })) eventValue)
