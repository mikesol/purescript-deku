...
instance Attr anything OnInvalid (Zora Unit) where
  attr OnInvalid value = unsafeAttribute { key: "invalid", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnInvalid (Zora Boolean) where
  attr OnInvalid value = unsafeAttribute { key: "invalid", value: cb' (Cb (const (runImpure value))) }