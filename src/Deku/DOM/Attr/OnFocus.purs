...
instance Attr anything OnFocus (Effect Unit) where
  attr OnFocus value = unsafeAttribute { key: "focus", value: cb' (Cb (const (value $> true))) }
instance Attr anything OnFocus (Effect Boolean) where
  attr OnFocus value = unsafeAttribute { key: "focus", value: cb' (Cb (const value)) }
instance Attr anything OnFocus (Zora Unit) where
  attr OnFocus value = unsafeAttribute { key: "focus", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnFocus (Zora Boolean) where
  attr OnFocus value = unsafeAttribute { key: "focus", value: cb' (Cb (const (runImpure value))) }