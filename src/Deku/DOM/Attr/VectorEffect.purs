...
instance Attr Circle_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr ClipPath_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr ClipPath_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr Defs_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr Defs_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr Ellipse_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr Ellipse_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr FeBlend_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr FeBlend_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr FeColorMatrix_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr FeColorMatrix_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr FeComponentTransfer_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr FeComponentTransfer_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr FeComposite_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr FeComposite_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr FeConvolveMatrix_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr FeConvolveMatrix_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr FeDiffuseLighting_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr FeDiffuseLighting_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr FeDisplacementMap_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr FeDisplacementMap_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr FeFlood_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr FeFlood_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr FeGaussianBlur_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr FeGaussianBlur_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr FeImage_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr FeImage_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr FeMerge_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr FeMerge_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr FeMorphology_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr FeMorphology_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr FeOffset_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr FeOffset_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr FeSpecularLighting_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr FeSpecularLighting_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr FeTile_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr FeTile_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr FeTurbulence_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr FeTurbulence_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr Filter_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr Filter_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr ForeignObject_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr ForeignObject_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr G_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr G_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr Image_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr Image_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr Line_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr Line_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr LinearGradient_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr LinearGradient_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr Marker_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr Marker_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr Mask_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr Mask_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr Path_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr Path_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr Pattern_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr Pattern_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr Polygon_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr Polygon_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr Polyline_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr Polyline_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr RadialGradient_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr RadialGradient_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr Rect_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr Rect_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr Svg_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr Svg_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr Switch_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr Switch_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr Symbol_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr Symbol_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr Text_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr Text_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr TextPath_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr TextPath_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr Tspan_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr Tspan_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr Use_ VectorEffect  String  where
  attr VectorEffect value = unsafeAttribute (  
    { key: "vector-effect", value: prop' value  } <$ _)
instance Attr Use_ VectorEffect (Event.Event Unit -> Event.Event  String ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \value -> { key: "vector-effect", value: prop' value })) eventValue)
...
instance Attr everything VectorEffect (Event.Event Unit -> Event.Event  Unit ) where
  attr VectorEffect eventValue = unsafeAttribute (map (map (
    \_ -> { key: "vector-effect", value: unset' })) eventValue)
