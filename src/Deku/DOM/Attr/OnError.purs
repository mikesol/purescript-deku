...
instance Attr anything OnError (Zora Unit) where
  attr OnError value = unsafeAttribute { key: "error", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnError (Zora Boolean) where
  attr OnError value = unsafeAttribute { key: "error", value: cb' (Cb (const (runImpure value))) }