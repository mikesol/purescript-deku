...
instance Attr anything OnPlaying (Effect Unit) where
  attr OnPlaying value = unsafeAttribute { key: "playing", value: cb' (Cb (const (value $> true))) }
instance Attr anything OnPlaying (Effect Boolean) where
  attr OnPlaying value = unsafeAttribute { key: "playing", value: cb' (Cb (const value)) }
instance Attr anything OnPlaying (Zora Unit) where
  attr OnPlaying value = unsafeAttribute { key: "playing", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnPlaying (Zora Boolean) where
  attr OnPlaying value = unsafeAttribute { key: "playing", value: cb' (Cb (const (runImpure value))) }