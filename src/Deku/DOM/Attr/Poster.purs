...
instance Attr Video_ Poster (Event.Event Unit -> Event.Event  String ) where
  attr Poster eventValue = unsafeAttribute (map (map (
    \value -> { key: "poster", value: prop' value })) eventValue)
...
instance Attr everything Poster (Event.Event Unit -> Event.Event  Unit ) where
  attr Poster eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "poster", value: unset' })) eventValue)
