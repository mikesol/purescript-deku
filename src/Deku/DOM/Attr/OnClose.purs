...
instance Attr anything OnClose (Event.Event Unit -> Event.Event  Cb ) where
  attr OnClose eventValue = unsafeAttribute (map (map (
    \value -> { key: "close", value: cb' value })) eventValue)
...
instance Attr anything OnClose (Event.Event Unit -> Event.Event  (Effect Unit) ) where
  attr OnClose eventValue = unsafeAttribute (map (map (
    \value -> { key: "close", value: cb' (Cb (const (value $> true))) })) eventValue)
...
instance Attr anything OnClose (Event.Event Unit -> Event.Event  (Effect Boolean) ) where
  attr OnClose eventValue = unsafeAttribute (map (map (
    \value -> { key: "close", value: cb' (Cb (const value)) })) eventValue) 
...
instance Attr everything OnClose (Event.Event Unit -> Event.Event  Unit ) where
  attr OnClose eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "close", value: unset' })) eventValue)
