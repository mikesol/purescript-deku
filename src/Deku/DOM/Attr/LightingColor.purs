...
instance Attr FeBlend_ LightingColor (Event.Event Unit -> Event.Event  String ) where
  attr LightingColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "lighting-color", value: prop' value })) eventValue)
...
instance Attr FeColorMatrix_ LightingColor  String  where
  attr LightingColor value = unsafeAttribute (  
    { key: "lighting-color", value: prop' value  } <$ _)
instance Attr FeColorMatrix_ LightingColor (Event.Event Unit -> Event.Event  String ) where
  attr LightingColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "lighting-color", value: prop' value })) eventValue)
...
instance Attr FeComponentTransfer_ LightingColor  String  where
  attr LightingColor value = unsafeAttribute (  
    { key: "lighting-color", value: prop' value  } <$ _)
instance Attr FeComponentTransfer_ LightingColor (Event.Event Unit -> Event.Event  String ) where
  attr LightingColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "lighting-color", value: prop' value })) eventValue)
...
instance Attr FeComposite_ LightingColor  String  where
  attr LightingColor value = unsafeAttribute (  
    { key: "lighting-color", value: prop' value  } <$ _)
instance Attr FeComposite_ LightingColor (Event.Event Unit -> Event.Event  String ) where
  attr LightingColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "lighting-color", value: prop' value })) eventValue)
...
instance Attr FeConvolveMatrix_ LightingColor  String  where
  attr LightingColor value = unsafeAttribute (  
    { key: "lighting-color", value: prop' value  } <$ _)
instance Attr FeConvolveMatrix_ LightingColor (Event.Event Unit -> Event.Event  String ) where
  attr LightingColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "lighting-color", value: prop' value })) eventValue)
...
instance Attr FeDiffuseLighting_ LightingColor  String  where
  attr LightingColor value = unsafeAttribute (  
    { key: "lighting-color", value: prop' value  } <$ _)
instance Attr FeDiffuseLighting_ LightingColor (Event.Event Unit -> Event.Event  String ) where
  attr LightingColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "lighting-color", value: prop' value })) eventValue)
...
instance Attr FeDisplacementMap_ LightingColor  String  where
  attr LightingColor value = unsafeAttribute (  
    { key: "lighting-color", value: prop' value  } <$ _)
instance Attr FeDisplacementMap_ LightingColor (Event.Event Unit -> Event.Event  String ) where
  attr LightingColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "lighting-color", value: prop' value })) eventValue)
...
instance Attr FeFlood_ LightingColor  String  where
  attr LightingColor value = unsafeAttribute (  
    { key: "lighting-color", value: prop' value  } <$ _)
instance Attr FeFlood_ LightingColor (Event.Event Unit -> Event.Event  String ) where
  attr LightingColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "lighting-color", value: prop' value })) eventValue)
...
instance Attr FeGaussianBlur_ LightingColor  String  where
  attr LightingColor value = unsafeAttribute (  
    { key: "lighting-color", value: prop' value  } <$ _)
instance Attr FeGaussianBlur_ LightingColor (Event.Event Unit -> Event.Event  String ) where
  attr LightingColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "lighting-color", value: prop' value })) eventValue)
...
instance Attr FeImage_ LightingColor  String  where
  attr LightingColor value = unsafeAttribute (  
    { key: "lighting-color", value: prop' value  } <$ _)
instance Attr FeImage_ LightingColor (Event.Event Unit -> Event.Event  String ) where
  attr LightingColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "lighting-color", value: prop' value })) eventValue)
...
instance Attr FeMerge_ LightingColor  String  where
  attr LightingColor value = unsafeAttribute (  
    { key: "lighting-color", value: prop' value  } <$ _)
instance Attr FeMerge_ LightingColor (Event.Event Unit -> Event.Event  String ) where
  attr LightingColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "lighting-color", value: prop' value })) eventValue)
...
instance Attr FeMorphology_ LightingColor  String  where
  attr LightingColor value = unsafeAttribute (  
    { key: "lighting-color", value: prop' value  } <$ _)
instance Attr FeMorphology_ LightingColor (Event.Event Unit -> Event.Event  String ) where
  attr LightingColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "lighting-color", value: prop' value })) eventValue)
...
instance Attr FeOffset_ LightingColor  String  where
  attr LightingColor value = unsafeAttribute (  
    { key: "lighting-color", value: prop' value  } <$ _)
instance Attr FeOffset_ LightingColor (Event.Event Unit -> Event.Event  String ) where
  attr LightingColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "lighting-color", value: prop' value })) eventValue)
...
instance Attr FeSpecularLighting_ LightingColor  String  where
  attr LightingColor value = unsafeAttribute (  
    { key: "lighting-color", value: prop' value  } <$ _)
instance Attr FeSpecularLighting_ LightingColor (Event.Event Unit -> Event.Event  String ) where
  attr LightingColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "lighting-color", value: prop' value })) eventValue)
...
instance Attr FeTile_ LightingColor  String  where
  attr LightingColor value = unsafeAttribute (  
    { key: "lighting-color", value: prop' value  } <$ _)
instance Attr FeTile_ LightingColor (Event.Event Unit -> Event.Event  String ) where
  attr LightingColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "lighting-color", value: prop' value })) eventValue)
...
instance Attr FeTurbulence_ LightingColor  String  where
  attr LightingColor value = unsafeAttribute (  
    { key: "lighting-color", value: prop' value  } <$ _)
instance Attr FeTurbulence_ LightingColor (Event.Event Unit -> Event.Event  String ) where
  attr LightingColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "lighting-color", value: prop' value })) eventValue)
...
instance Attr Filter_ LightingColor  String  where
  attr LightingColor value = unsafeAttribute (  
    { key: "lighting-color", value: prop' value  } <$ _)
instance Attr Filter_ LightingColor (Event.Event Unit -> Event.Event  String ) where
  attr LightingColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "lighting-color", value: prop' value })) eventValue)
...
instance Attr Image_ LightingColor  String  where
  attr LightingColor value = unsafeAttribute (  
    { key: "lighting-color", value: prop' value  } <$ _)
instance Attr Image_ LightingColor (Event.Event Unit -> Event.Event  String ) where
  attr LightingColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "lighting-color", value: prop' value })) eventValue)
...
instance Attr Switch_ LightingColor  String  where
  attr LightingColor value = unsafeAttribute (  
    { key: "lighting-color", value: prop' value  } <$ _)
instance Attr Switch_ LightingColor (Event.Event Unit -> Event.Event  String ) where
  attr LightingColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "lighting-color", value: prop' value })) eventValue)
...
instance Attr everything LightingColor (Event.Event Unit -> Event.Event  Unit ) where
  attr LightingColor eventValue = unsafeAttribute (map (map (
    \_ -> { key: "lighting-color", value: unset' })) eventValue)
