...
instance Attr FeBlend_ StopColor  String  where
  attr StopColor value = unsafeAttribute (  
    { key: "stop-color", value: prop' value  } <$ _)
instance Attr FeBlend_ StopColor (Event.Event Unit -> Event.Event  String ) where
  attr StopColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-color", value: prop' value })) eventValue)
instance Attr FeBlend_ StopColor (Event.Event  String ) where
  attr StopColor eventValue = unsafeAttribute \_ -> eventValue <#>
    \value -> { key: "stop-color", value: prop' value }
instance Attr FeColorMatrix_ StopColor  String  where
  attr StopColor value = unsafeAttribute (  
    { key: "stop-color", value: prop' value  } <$ _)
instance Attr FeColorMatrix_ StopColor (Event.Event Unit -> Event.Event  String ) where
  attr StopColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-color", value: prop' value })) eventValue)
...
instance Attr FeComponentTransfer_ StopColor  String  where
  attr StopColor value = unsafeAttribute (  
    { key: "stop-color", value: prop' value  } <$ _)
instance Attr FeComponentTransfer_ StopColor (Event.Event Unit -> Event.Event  String ) where
  attr StopColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-color", value: prop' value })) eventValue)
...
instance Attr FeComposite_ StopColor  String  where
  attr StopColor value = unsafeAttribute (  
    { key: "stop-color", value: prop' value  } <$ _)
instance Attr FeComposite_ StopColor (Event.Event Unit -> Event.Event  String ) where
  attr StopColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-color", value: prop' value })) eventValue)
...
instance Attr FeConvolveMatrix_ StopColor  String  where
  attr StopColor value = unsafeAttribute (  
    { key: "stop-color", value: prop' value  } <$ _)
instance Attr FeConvolveMatrix_ StopColor (Event.Event Unit -> Event.Event  String ) where
  attr StopColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-color", value: prop' value })) eventValue)
...
instance Attr FeDiffuseLighting_ StopColor  String  where
  attr StopColor value = unsafeAttribute (  
    { key: "stop-color", value: prop' value  } <$ _)
instance Attr FeDiffuseLighting_ StopColor (Event.Event Unit -> Event.Event  String ) where
  attr StopColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-color", value: prop' value })) eventValue)
...
instance Attr FeDisplacementMap_ StopColor  String  where
  attr StopColor value = unsafeAttribute (  
    { key: "stop-color", value: prop' value  } <$ _)
instance Attr FeDisplacementMap_ StopColor (Event.Event Unit -> Event.Event  String ) where
  attr StopColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-color", value: prop' value })) eventValue)
...
instance Attr FeFlood_ StopColor  String  where
  attr StopColor value = unsafeAttribute (  
    { key: "stop-color", value: prop' value  } <$ _)
instance Attr FeFlood_ StopColor (Event.Event Unit -> Event.Event  String ) where
  attr StopColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-color", value: prop' value })) eventValue)
...
instance Attr FeGaussianBlur_ StopColor  String  where
  attr StopColor value = unsafeAttribute (  
    { key: "stop-color", value: prop' value  } <$ _)
instance Attr FeGaussianBlur_ StopColor (Event.Event Unit -> Event.Event  String ) where
  attr StopColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-color", value: prop' value })) eventValue)
...
instance Attr FeImage_ StopColor  String  where
  attr StopColor value = unsafeAttribute (  
    { key: "stop-color", value: prop' value  } <$ _)
instance Attr FeImage_ StopColor (Event.Event Unit -> Event.Event  String ) where
  attr StopColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-color", value: prop' value })) eventValue)
...
instance Attr FeMerge_ StopColor  String  where
  attr StopColor value = unsafeAttribute (  
    { key: "stop-color", value: prop' value  } <$ _)
instance Attr FeMerge_ StopColor (Event.Event Unit -> Event.Event  String ) where
  attr StopColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-color", value: prop' value })) eventValue)
...
instance Attr FeMorphology_ StopColor  String  where
  attr StopColor value = unsafeAttribute (  
    { key: "stop-color", value: prop' value  } <$ _)
instance Attr FeMorphology_ StopColor (Event.Event Unit -> Event.Event  String ) where
  attr StopColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-color", value: prop' value })) eventValue)
...
instance Attr FeOffset_ StopColor  String  where
  attr StopColor value = unsafeAttribute (  
    { key: "stop-color", value: prop' value  } <$ _)
instance Attr FeOffset_ StopColor (Event.Event Unit -> Event.Event  String ) where
  attr StopColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-color", value: prop' value })) eventValue)
...
instance Attr FeSpecularLighting_ StopColor  String  where
  attr StopColor value = unsafeAttribute (  
    { key: "stop-color", value: prop' value  } <$ _)
instance Attr FeSpecularLighting_ StopColor (Event.Event Unit -> Event.Event  String ) where
  attr StopColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-color", value: prop' value })) eventValue)
...
instance Attr FeTile_ StopColor  String  where
  attr StopColor value = unsafeAttribute (  
    { key: "stop-color", value: prop' value  } <$ _)
instance Attr FeTile_ StopColor (Event.Event Unit -> Event.Event  String ) where
  attr StopColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-color", value: prop' value })) eventValue)
...
instance Attr FeTurbulence_ StopColor  String  where
  attr StopColor value = unsafeAttribute (  
    { key: "stop-color", value: prop' value  } <$ _)
instance Attr FeTurbulence_ StopColor (Event.Event Unit -> Event.Event  String ) where
  attr StopColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-color", value: prop' value })) eventValue)
...
instance Attr Filter_ StopColor  String  where
  attr StopColor value = unsafeAttribute (  
    { key: "stop-color", value: prop' value  } <$ _)
instance Attr Filter_ StopColor (Event.Event Unit -> Event.Event  String ) where
  attr StopColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-color", value: prop' value })) eventValue)
...
instance Attr Image_ StopColor  String  where
  attr StopColor value = unsafeAttribute (  
    { key: "stop-color", value: prop' value  } <$ _)
instance Attr Image_ StopColor (Event.Event Unit -> Event.Event  String ) where
  attr StopColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-color", value: prop' value })) eventValue)
...
instance Attr Stop_ StopColor  String  where
  attr StopColor value = unsafeAttribute (  
    { key: "stop-color", value: prop' value  } <$ _)
instance Attr Stop_ StopColor (Event.Event Unit -> Event.Event  String ) where
  attr StopColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-color", value: prop' value })) eventValue)
...
instance Attr Switch_ StopColor  String  where
  attr StopColor value = unsafeAttribute (  
    { key: "stop-color", value: prop' value  } <$ _)
instance Attr Switch_ StopColor (Event.Event Unit -> Event.Event  String ) where
  attr StopColor eventValue = unsafeAttribute (map (map (
    \value -> { key: "stop-color", value: prop' value })) eventValue)
...
instance Attr everything StopColor  Unit  where
  attr StopColor _ = unsafeAttribute (  
    { key: "stop-color", value: unset'  } <$ _)
instance Attr everything StopColor (Event.Event Unit -> Event.Event  Unit ) where
  attr StopColor eventValue = unsafeAttribute (map (map (
    \_ -> { key: "stop-color", value: unset' })) eventValue)
