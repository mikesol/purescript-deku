...
instance Attr anything OnTransitionend (Zora Unit) where
  attr OnTransitionend value = unsafeAttribute { key: "transitionend", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnTransitionend (Zora Boolean) where
  attr OnTransitionend value = unsafeAttribute { key: "transitionend", value: cb' (Cb (const (runImpure value))) }