...
instance Attr anything OnTransitionend (Event.Event Unit -> Event.Event  Cb ) where
  attr OnTransitionend eventValue = unsafeAttribute (map (map ( \value -> { key: "transitionend", value: cb' value })) eventValue)
...
instance Attr anything OnTransitionend (Event.Event Unit -> Event.Event  (Effect Unit) ) where
  attr OnTransitionend eventValue = unsafeAttribute (map (map ( \value ->
      { key: "transitionend", value: cb' (Cb (const (value $> true))) })) eventValue)
...
instance Attr anything OnTransitionend (Event.Event Unit -> Event.Event  (Effect Boolean) ) where
  attr OnTransitionend eventValue = unsafeAttribute (map (map ( \value -> { key: "transitionend", value: cb' (Cb (const value)) })) eventValue) 
...
instance Attr everything OnTransitionend (Event.Event Unit -> Event.Event  Unit ) where
  attr OnTransitionend eventValue = unsafeAttribute (map (map ( \_ -> { key: "transitionend", value: unset' })) eventValue)
