...
instance Attr anything OnDragend (Event.Event Unit -> Event.Event  Cb ) where
  attr OnDragend eventValue = unsafeAttribute (map (map (
    \value -> { key: "dragend", value: cb' value })) eventValue)
...
instance Attr anything OnDragend (Event.Event Unit -> Event.Event  (Effect Unit) ) where
  attr OnDragend eventValue = unsafeAttribute (map (map (
    \value -> { key: "dragend", value: cb' (Cb (const (value $> true))) })) eventValue)
...
instance Attr anything OnDragend (Event.Event Unit -> Event.Event  (Effect Boolean) ) where
  attr OnDragend eventValue = unsafeAttribute (map (map (
    \value -> { key: "dragend", value: cb' (Cb (const value)) })) eventValue) 
...
instance Attr everything OnDragend (Event.Event Unit -> Event.Event  Unit ) where
  attr OnDragend eventValue = unsafeAttribute (map (map (
    \_ -> { key: "dragend", value: unset' })) eventValue)
