...
instance Attr anything OnMouseenter (Zora Unit) where
  attr OnMouseenter value = unsafeAttribute { key: "mouseenter", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnMouseenter (Zora Boolean) where
  attr OnMouseenter value = unsafeAttribute { key: "mouseenter", value: cb' (Cb (const (runImpure value))) }