...
instance Attr Svg_ ContentStyleType (Event.Event Unit -> Event.Event  String ) where
  attr ContentStyleType eventValue = unsafeAttribute (map (map ( \value -> { key: "contentStyleType", value: prop' value })) eventValue)
...
instance Attr everything ContentStyleType (Event.Event Unit -> Event.Event  Unit ) where
  attr ContentStyleType eventValue = unsafeAttribute (map (map ( \_ -> { key: "contentStyleType", value: unset' })) eventValue)
