...
instance Attr anything OnAnimationstart  (Zora Unit) where
  attr OnAnimationstart  value = unsafeAttribute { key: "animationstart ", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnAnimationstart  (Zora Boolean) where
  attr OnAnimationstart  value = unsafeAttribute { key: "animationstart ", value: cb' (Cb (const (runImpure value))) }