...
instance Attr FeDropShadow_ StdDeviation (Event.Event Unit -> Event.Event  String ) where
  attr StdDeviation eventValue = unsafeAttribute (map (map (
    \value -> { key: "stdDeviation", value: prop' value })) eventValue)
...
instance Attr FeGaussianBlur_ StdDeviation  String  where
  attr StdDeviation value = unsafeAttribute (  
    { key: "stdDeviation", value: prop' value  } <$ _)
instance Attr FeGaussianBlur_ StdDeviation (Event.Event Unit -> Event.Event  String ) where
  attr StdDeviation eventValue = unsafeAttribute (map (map (
    \value -> { key: "stdDeviation", value: prop' value })) eventValue)
...
instance Attr everything StdDeviation (Event.Event Unit -> Event.Event  Unit ) where
  attr StdDeviation eventValue = unsafeAttribute (map (map (
    \_ -> { key: "stdDeviation", value: unset' })) eventValue)
