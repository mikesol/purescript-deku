...
instance Attr anything OnShow (Event.Event Unit -> Event.Event  Cb ) where
  attr OnShow eventValue = unsafeAttribute (map (map (
    \value -> { key: "show", value: cb' value })) eventValue)
...
instance Attr anything OnShow (Event.Event Unit -> Event.Event  (Effect Unit) ) where
  attr OnShow eventValue = unsafeAttribute (map (map (
    \value -> { key: "show", value: cb' (Cb (const (value $> true))) })) eventValue)
...
instance Attr anything OnShow (Event.Event Unit -> Event.Event  (Effect Boolean) ) where
  attr OnShow eventValue = unsafeAttribute (map (map (
    \value -> { key: "show", value: cb' (Cb (const value)) })) eventValue) 
...
instance Attr everything OnShow (Event.Event Unit -> Event.Event  Unit ) where
  attr OnShow eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "show", value: unset' })) eventValue)
