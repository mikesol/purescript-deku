...
instance Attr anything OnSlotchange (Zora Unit) where
  attr OnSlotchange value = unsafeAttribute { key: "slotchange", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnSlotchange (Zora Boolean) where
  attr OnSlotchange value = unsafeAttribute { key: "slotchange", value: cb' (Cb (const (runImpure value))) }