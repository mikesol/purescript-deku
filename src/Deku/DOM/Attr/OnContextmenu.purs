...
instance Attr anything OnContextmenu (Zora Unit) where
  attr OnContextmenu value = unsafeAttribute { key: "contextmenu", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnContextmenu (Zora Boolean) where
  attr OnContextmenu value = unsafeAttribute { key: "contextmenu", value: cb' (Cb (const (runImpure value))) }