...
instance Attr FeComposite_ K3 (Event.Event Unit -> Event.Event  String ) where
  attr K3 eventValue = unsafeAttribute (map (map ( \value ->
    { key: "k3", value: prop' value })) eventValue)
...
instance Attr everything K3 (Event.Event Unit -> Event.Event  Unit ) where
  attr K3 eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "k3", value: unset' })) eventValue)
