...
instance Attr anything OnSuspend (Event.Event Unit -> Event.Event  Cb ) where
  attr OnSuspend eventValue = unsafeAttribute (map (map (
    \value -> { key: "suspend", value: cb' value })) eventValue)
...
instance Attr anything OnSuspend (Event.Event Unit -> Event.Event  (Effect Unit) ) where
  attr OnSuspend eventValue = unsafeAttribute (map (map (
    \value -> { key: "suspend", value: cb' (Cb (const (value $> true))) })) eventValue)
...
instance Attr anything OnSuspend (Event.Event Unit -> Event.Event  (Effect Boolean) ) where
  attr OnSuspend eventValue = unsafeAttribute (map (map (
    \value -> { key: "suspend", value: cb' (Cb (const value)) })) eventValue) 
...
instance Attr everything OnSuspend (Event.Event Unit -> Event.Event  Unit ) where
  attr OnSuspend eventValue = unsafeAttribute (map (map (
    \_ -> { key: "suspend", value: unset' })) eventValue)
