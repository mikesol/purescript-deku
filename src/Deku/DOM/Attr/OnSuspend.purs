...
instance Attr anything OnSuspend (Zora Unit) where
  attr OnSuspend value = unsafeAttribute { key: "suspend", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnSuspend (Zora Boolean) where
  attr OnSuspend value = unsafeAttribute { key: "suspend", value: cb' (Cb (const (runImpure value))) }