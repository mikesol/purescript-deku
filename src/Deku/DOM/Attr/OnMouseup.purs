...
instance Attr anything OnMouseup (Event.Event Unit -> Event.Event  Cb ) where
  attr OnMouseup eventValue = unsafeAttribute (map (map (
    \value -> { key: "mouseup", value: cb' value })) eventValue)
...
instance Attr anything OnMouseup (Event.Event Unit -> Event.Event  (Effect Unit) ) where
  attr OnMouseup eventValue = unsafeAttribute (map (map (
    \value -> { key: "mouseup", value: cb' (Cb (const (value $> true))) })) eventValue)
...
instance Attr anything OnMouseup (Event.Event Unit -> Event.Event  (Effect Boolean) ) where
  attr OnMouseup eventValue = unsafeAttribute (map (map (
    \value -> { key: "mouseup", value: cb' (Cb (const value)) })) eventValue) 
...
instance Attr everything OnMouseup (Event.Event Unit -> Event.Event  Unit ) where
  attr OnMouseup eventValue = unsafeAttribute (map (map (
    \_ -> { key: "mouseup", value: unset' })) eventValue)
