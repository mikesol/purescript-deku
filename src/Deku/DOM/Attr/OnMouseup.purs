...
instance Attr anything OnMouseup (Zora Unit) where
  attr OnMouseup value = unsafeAttribute { key: "mouseup", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnMouseup (Zora Boolean) where
  attr OnMouseup value = unsafeAttribute { key: "mouseup", value: cb' (Cb (const (runImpure value))) }