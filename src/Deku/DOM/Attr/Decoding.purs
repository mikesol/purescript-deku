...
instance Attr Img_ Decoding (Event.Event Unit -> Event.Event  String ) where
  attr Decoding eventValue = unsafeAttribute (map (map (
    \value -> { key: "decoding", value: prop' value })) eventValue)
...
instance Attr everything Decoding (Event.Event Unit -> Event.Event  Unit ) where
  attr Decoding eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "decoding", value: unset' })) eventValue)
