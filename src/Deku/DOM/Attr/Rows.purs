...
instance Attr Textarea_ Rows (Event.Event Unit -> Event.Event  String ) where
  attr Rows eventValue = unsafeAttribute (map (map ( \value ->
    { key: "rows", value: prop' value })) eventValue)
...
instance Attr everything Rows (Event.Event Unit -> Event.Event  Unit ) where
  attr Rows eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "rows", value: unset' })) eventValue)
