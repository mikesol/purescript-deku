...
instance Attr Optgroup_ Label (Event.Event Unit -> Event.Event  String ) where
  attr Label eventValue = unsafeAttribute (map (map (
    \value -> { key: "label", value: prop' value })) eventValue)
...
instance Attr Option_ Label  String  where
  attr Label value = unsafeAttribute (  
    { key: "label", value: prop' value  } <$ _)
instance Attr Option_ Label (Event.Event Unit -> Event.Event  String ) where
  attr Label eventValue = unsafeAttribute (map (map (
    \value -> { key: "label", value: prop' value })) eventValue)
...
instance Attr Track_ Label  String  where
  attr Label value = unsafeAttribute (  
    { key: "label", value: prop' value  } <$ _)
instance Attr Track_ Label (Event.Event Unit -> Event.Event  String ) where
  attr Label eventValue = unsafeAttribute (map (map (
    \value -> { key: "label", value: prop' value })) eventValue)
...
instance Attr everything Label (Event.Event Unit -> Event.Event  Unit ) where
  attr Label eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "label", value: unset' })) eventValue)
