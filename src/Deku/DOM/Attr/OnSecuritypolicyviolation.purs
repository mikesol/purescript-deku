...
instance Attr anything OnSecuritypolicyviolation (Event.Event Unit -> Event.Event  Cb ) where
  attr OnSecuritypolicyviolation eventValue = unsafeAttribute (map (map ( \value ->
      { key: "securitypolicyviolation", value: cb' value })) eventValue)
...
instance Attr anything OnSecuritypolicyviolation (Event.Event Unit -> Event.Event  (Effect Unit) ) where
  attr OnSecuritypolicyviolation eventValue = unsafeAttribute (map (map ( \value ->
...
instance Attr anything OnSecuritypolicyviolation (Event.Event Unit -> Event.Event  (Effect Boolean) ) where
  attr OnSecuritypolicyviolation eventValue = unsafeAttribute (map (map ( \value ->
      { key: "securitypolicyviolation", value: cb' (Cb (const value)) })) eventValue) 
...
instance Attr everything OnSecuritypolicyviolation (Event.Event Unit -> Event.Event  Unit ) where
  attr OnSecuritypolicyviolation eventValue = unsafeAttribute (map (map ( \_ -> { key: "securitypolicyviolation", value: unset' })) eventValue)
