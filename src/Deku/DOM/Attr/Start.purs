...
instance Attr Ol_ Start (Event.Event Unit -> Event.Event  String ) where
  attr Start eventValue = unsafeAttribute (map (map (
    \value -> { key: "start", value: prop' value })) eventValue)
...
instance Attr everything Start (Event.Event Unit -> Event.Event  Unit ) where
  attr Start eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "start", value: unset' })) eventValue)
