...
instance Attr Input_ Dirname (Event.Event Unit -> Event.Event  String ) where
  attr Dirname eventValue = unsafeAttribute (map (map (
    \value -> { key: "dirname", value: prop' value })) eventValue)
...
instance Attr Textarea_ Dirname  String  where
  attr Dirname value = unsafeAttribute (  
    { key: "dirname", value: prop' value  } <$ _)
instance Attr Textarea_ Dirname (Event.Event Unit -> Event.Event  String ) where
  attr Dirname eventValue = unsafeAttribute (map (map (
    \value -> { key: "dirname", value: prop' value })) eventValue)
...
instance Attr everything Dirname (Event.Event Unit -> Event.Event  Unit ) where
  attr Dirname eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "dirname", value: unset' })) eventValue)
