...
instance Attr AnimateTransform_ XlinkActuate (Event.Event Unit -> Event.Event  String ) where
  attr XlinkActuate eventValue = unsafeAttribute (map (map (
    \value -> { key: "xlink:actuate", value: prop' value })) eventValue)
...
instance Attr FeImage_ XlinkActuate  String  where
  attr XlinkActuate value = unsafeAttribute (  
    { key: "xlink:actuate", value: prop' value  } <$ _)
instance Attr FeImage_ XlinkActuate (Event.Event Unit -> Event.Event  String ) where
  attr XlinkActuate eventValue = unsafeAttribute (map (map (
    \value -> { key: "xlink:actuate", value: prop' value })) eventValue)
...
instance Attr Filter_ XlinkActuate  String  where
  attr XlinkActuate value = unsafeAttribute (  
    { key: "xlink:actuate", value: prop' value  } <$ _)
instance Attr Filter_ XlinkActuate (Event.Event Unit -> Event.Event  String ) where
  attr XlinkActuate eventValue = unsafeAttribute (map (map (
    \value -> { key: "xlink:actuate", value: prop' value })) eventValue)
...
instance Attr Image_ XlinkActuate  String  where
  attr XlinkActuate value = unsafeAttribute (  
    { key: "xlink:actuate", value: prop' value  } <$ _)
instance Attr Image_ XlinkActuate (Event.Event Unit -> Event.Event  String ) where
  attr XlinkActuate eventValue = unsafeAttribute (map (map (
    \value -> { key: "xlink:actuate", value: prop' value })) eventValue)
...
instance Attr Mpath_ XlinkActuate  String  where
  attr XlinkActuate value = unsafeAttribute (  
    { key: "xlink:actuate", value: prop' value  } <$ _)
instance Attr Mpath_ XlinkActuate (Event.Event Unit -> Event.Event  String ) where
  attr XlinkActuate eventValue = unsafeAttribute (map (map (
    \value -> { key: "xlink:actuate", value: prop' value })) eventValue)
...
instance Attr everything XlinkActuate (Event.Event Unit -> Event.Event  Unit ) where
  attr XlinkActuate eventValue = unsafeAttribute (map (map (
    \_ -> { key: "xlink:actuate", value: unset' })) eventValue)
