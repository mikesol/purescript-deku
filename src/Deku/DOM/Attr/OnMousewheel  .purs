...
instance Attr anything OnMousewheel   (Effect Unit) where
  attr OnMousewheel   value = unsafeAttribute { key: "mousewheel  ", value: cb' (Cb (const (value $> true))) }
instance Attr anything OnMousewheel   (Effect Boolean) where
  attr OnMousewheel   value = unsafeAttribute { key: "mousewheel  ", value: cb' (Cb (const value)) }
instance Attr anything OnMousewheel   (Zora Unit) where
  attr OnMousewheel   value = unsafeAttribute { key: "mousewheel  ", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnMousewheel   (Zora Boolean) where
  attr OnMousewheel   value = unsafeAttribute { key: "mousewheel  ", value: cb' (Cb (const (runImpure value))) }