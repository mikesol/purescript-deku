...
instance Attr FeDropShadow_ Dy (Event.Event Unit -> Event.Event  String ) where
  attr Dy eventValue = unsafeAttribute (map (map ( \value ->
    { key: "dy", value: prop' value })) eventValue)
...
instance Attr FeOffset_ Dy  String  where
  attr Dy value = unsafeAttribute (  { key: "dy", value: prop' value  } <$ _)
instance Attr FeOffset_ Dy (Event.Event Unit -> Event.Event  String ) where
  attr Dy eventValue = unsafeAttribute (map (map ( \value ->
    { key: "dy", value: prop' value })) eventValue)
...
instance Attr Text_ Dy  String  where
  attr Dy value = unsafeAttribute (  { key: "dy", value: prop' value  } <$ _)
instance Attr Text_ Dy (Event.Event Unit -> Event.Event  String ) where
  attr Dy eventValue = unsafeAttribute (map (map ( \value ->
    { key: "dy", value: prop' value })) eventValue)
...
instance Attr Tspan_ Dy  String  where
  attr Dy value = unsafeAttribute (  { key: "dy", value: prop' value  } <$ _)
instance Attr Tspan_ Dy (Event.Event Unit -> Event.Event  String ) where
  attr Dy eventValue = unsafeAttribute (map (map ( \value ->
    { key: "dy", value: prop' value })) eventValue)
...
instance Attr everything Dy (Event.Event Unit -> Event.Event  Unit ) where
  attr Dy eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "dy", value: unset' })) eventValue)
