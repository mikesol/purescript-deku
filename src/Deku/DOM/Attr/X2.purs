...
instance Attr Line_ X2 (Event.Event Unit -> Event.Event  String ) where
  attr X2 eventValue = unsafeAttribute (map (map ( \value ->
    { key: "x2", value: prop' value })) eventValue)
...
instance Attr LinearGradient_ X2  String  where
  attr X2 value = unsafeAttribute (  { key: "x2", value: prop' value  } <$ _)
instance Attr LinearGradient_ X2 (Event.Event Unit -> Event.Event  String ) where
  attr X2 eventValue = unsafeAttribute (map (map ( \value ->
    { key: "x2", value: prop' value })) eventValue)
...
instance Attr everything X2 (Event.Event Unit -> Event.Event  Unit ) where
  attr X2 eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "x2", value: unset' })) eventValue)
