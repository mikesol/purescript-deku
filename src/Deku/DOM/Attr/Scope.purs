...
instance Attr Th_ Scope (Event.Event Unit -> Event.Event  String ) where
  attr Scope eventValue = unsafeAttribute (map (map (
    \value -> { key: "scope", value: prop' value })) eventValue)
...
instance Attr everything Scope (Event.Event Unit -> Event.Event  Unit ) where
  attr Scope eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "scope", value: unset' })) eventValue)
