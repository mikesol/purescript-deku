...
instance Attr FeTurbulence_ Seed (Event.Event Unit -> Event.Event  String ) where
  attr Seed eventValue = unsafeAttribute (map (map ( \value ->
    { key: "seed", value: prop' value })) eventValue)
...
instance Attr everything Seed (Event.Event Unit -> Event.Event  Unit ) where
  attr Seed eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "seed", value: unset' })) eventValue)
