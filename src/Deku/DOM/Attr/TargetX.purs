...
instance Attr FeConvolveMatrix_ TargetX (Event.Event Unit -> Event.Event  String ) where
  attr TargetX eventValue = unsafeAttribute (map (map (
    \value -> { key: "targetX", value: prop' value })) eventValue)
...
instance Attr everything TargetX (Event.Event Unit -> Event.Event  Unit ) where
  attr TargetX eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "targetX", value: unset' })) eventValue)
