...
instance Attr Iframe_ Csp (Event.Event Unit -> Event.Event  String ) where
  attr Csp eventValue = unsafeAttribute (map (map ( \value ->
    { key: "csp", value: prop' value })) eventValue)
...
instance Attr everything Csp (Event.Event Unit -> Event.Event  Unit ) where
  attr Csp eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "csp", value: unset' })) eventValue)
