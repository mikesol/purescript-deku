...
instance Attr FeMorphology_ Radius (Event.Event Unit -> Event.Event  String ) where
  attr Radius eventValue = unsafeAttribute (map (map (
    \value -> { key: "radius", value: prop' value })) eventValue)
...
instance Attr everything Radius (Event.Event Unit -> Event.Event  Unit ) where
  attr Radius eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "radius", value: unset' })) eventValue)
