...
instance Attr Meter_ Low (Event.Event Unit -> Event.Event  String ) where
  attr Low eventValue = unsafeAttribute (map (map ( \value ->
    { key: "low", value: prop' value })) eventValue)
...
instance Attr everything Low (Event.Event Unit -> Event.Event  Unit ) where
  attr Low eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "low", value: unset' })) eventValue)
