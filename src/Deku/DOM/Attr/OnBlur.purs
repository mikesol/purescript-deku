...
instance Attr anything OnBlur (Event.Event Unit -> Event.Event  Cb ) where
  attr OnBlur eventValue = unsafeAttribute (map (map (
    \value -> { key: "blur", value: cb' value })) eventValue)
...
instance Attr anything OnBlur (Event.Event Unit -> Event.Event  (Effect Unit) ) where
  attr OnBlur eventValue = unsafeAttribute (map (map (
    \value -> { key: "blur", value: cb' (Cb (const (value $> true))) })) eventValue)
...
instance Attr anything OnBlur (Event.Event Unit -> Event.Event  (Effect Boolean) ) where
  attr OnBlur eventValue = unsafeAttribute (map (map (
    \value -> { key: "blur", value: cb' (Cb (const value)) })) eventValue) 
...
instance Attr everything OnBlur (Event.Event Unit -> Event.Event  Unit ) where
  attr OnBlur eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "blur", value: unset' })) eventValue)
