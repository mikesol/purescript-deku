...
instance Attr Track_ Default (Event.Event Unit -> Event.Event  String ) where
  attr Default eventValue = unsafeAttribute (map (map (
    \value -> { key: "default", value: prop' value })) eventValue)
...
instance Attr everything Default (Event.Event Unit -> Event.Event  Unit ) where
  attr Default eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "default", value: unset' })) eventValue)
