...
instance Attr FeTurbulence_ StitchTiles (Event.Event Unit -> Event.Event  String ) where
  attr StitchTiles eventValue = unsafeAttribute (map (map (
    \value -> { key: "stitchTiles", value: prop' value })) eventValue)
...
instance Attr everything StitchTiles (Event.Event Unit -> Event.Event  Unit ) where
  attr StitchTiles eventValue = unsafeAttribute (map (map (
    \_ -> { key: "stitchTiles", value: unset' })) eventValue)
