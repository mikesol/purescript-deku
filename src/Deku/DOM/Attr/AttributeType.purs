...
instance Attr AnimateTransform_ AttributeType (Event.Event Unit -> Event.Event  String ) where
  attr AttributeType eventValue = unsafeAttribute (map (map (
    \value -> { key: "attributeType", value: prop' value })) eventValue)
...
instance Attr everything AttributeType (Event.Event Unit -> Event.Event  Unit ) where
  attr AttributeType eventValue = unsafeAttribute (map (map (
    \_ -> { key: "attributeType", value: unset' })) eventValue)
