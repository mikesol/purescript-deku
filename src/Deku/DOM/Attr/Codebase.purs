...
instance Attr Applet_ Codebase (Event.Event Unit -> Event.Event  String ) where
  attr Codebase eventValue = unsafeAttribute (map (map (
    \value -> { key: "codebase", value: prop' value })) eventValue)
...
instance Attr everything Codebase (Event.Event Unit -> Event.Event  Unit ) where
  attr Codebase eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "codebase", value: unset' })) eventValue)
