...
instance Attr A_ Ping (Event.Event Unit -> Event.Event  String ) where
  attr Ping eventValue = unsafeAttribute (map (map ( \value ->
    { key: "ping", value: prop' value })) eventValue)
...
instance Attr Area_ Ping  String  where
  attr Ping value = unsafeAttribute (  
    { key: "ping", value: prop' value  } <$ _)
instance Attr Area_ Ping (Event.Event Unit -> Event.Event  String ) where
  attr Ping eventValue = unsafeAttribute (map (map ( \value ->
    { key: "ping", value: prop' value })) eventValue)
...
instance Attr everything Ping (Event.Event Unit -> Event.Event  Unit ) where
  attr Ping eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "ping", value: unset' })) eventValue)
