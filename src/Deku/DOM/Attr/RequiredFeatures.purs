...
instance Attr AnimateTransform_ RequiredFeatures (Event.Event Unit -> Event.Event  String ) where
  attr RequiredFeatures eventValue = unsafeAttribute (map (map ( \value -> { key: "requiredFeatures", value: prop' value })) eventValue)
...
instance Attr Discard_ RequiredFeatures  String  where
  attr RequiredFeatures value = unsafeAttribute (  
    { key: "requiredFeatures", value: prop' value  } <$ _)
instance Attr Discard_ RequiredFeatures (Event.Event Unit -> Event.Event  String ) where
  attr RequiredFeatures eventValue = unsafeAttribute (map (map ( \value -> { key: "requiredFeatures", value: prop' value })) eventValue)
...
instance Attr Ellipse_ RequiredFeatures  String  where
  attr RequiredFeatures value = unsafeAttribute (  
    { key: "requiredFeatures", value: prop' value  } <$ _)
instance Attr Ellipse_ RequiredFeatures (Event.Event Unit -> Event.Event  String ) where
  attr RequiredFeatures eventValue = unsafeAttribute (map (map ( \value -> { key: "requiredFeatures", value: prop' value })) eventValue)
...
instance Attr ForeignObject_ RequiredFeatures  String  where
  attr RequiredFeatures value = unsafeAttribute (  
    { key: "requiredFeatures", value: prop' value  } <$ _)
instance Attr ForeignObject_ RequiredFeatures (Event.Event Unit -> Event.Event  String ) where
  attr RequiredFeatures eventValue = unsafeAttribute (map (map ( \value -> { key: "requiredFeatures", value: prop' value })) eventValue)
...
instance Attr G_ RequiredFeatures  String  where
  attr RequiredFeatures value = unsafeAttribute (  
    { key: "requiredFeatures", value: prop' value  } <$ _)
instance Attr G_ RequiredFeatures (Event.Event Unit -> Event.Event  String ) where
  attr RequiredFeatures eventValue = unsafeAttribute (map (map ( \value -> { key: "requiredFeatures", value: prop' value })) eventValue)
...
instance Attr Image_ RequiredFeatures  String  where
  attr RequiredFeatures value = unsafeAttribute (  
    { key: "requiredFeatures", value: prop' value  } <$ _)
instance Attr Image_ RequiredFeatures (Event.Event Unit -> Event.Event  String ) where
  attr RequiredFeatures eventValue = unsafeAttribute (map (map ( \value -> { key: "requiredFeatures", value: prop' value })) eventValue)
...
instance Attr Line_ RequiredFeatures  String  where
  attr RequiredFeatures value = unsafeAttribute (  
    { key: "requiredFeatures", value: prop' value  } <$ _)
instance Attr Line_ RequiredFeatures (Event.Event Unit -> Event.Event  String ) where
  attr RequiredFeatures eventValue = unsafeAttribute (map (map ( \value -> { key: "requiredFeatures", value: prop' value })) eventValue)
...
instance Attr Marker_ RequiredFeatures  String  where
  attr RequiredFeatures value = unsafeAttribute (  
    { key: "requiredFeatures", value: prop' value  } <$ _)
instance Attr Marker_ RequiredFeatures (Event.Event Unit -> Event.Event  String ) where
  attr RequiredFeatures eventValue = unsafeAttribute (map (map ( \value -> { key: "requiredFeatures", value: prop' value })) eventValue)
...
instance Attr Mask_ RequiredFeatures  String  where
  attr RequiredFeatures value = unsafeAttribute (  
    { key: "requiredFeatures", value: prop' value  } <$ _)
instance Attr Mask_ RequiredFeatures (Event.Event Unit -> Event.Event  String ) where
  attr RequiredFeatures eventValue = unsafeAttribute (map (map ( \value -> { key: "requiredFeatures", value: prop' value })) eventValue)
...
instance Attr Path_ RequiredFeatures  String  where
  attr RequiredFeatures value = unsafeAttribute (  
    { key: "requiredFeatures", value: prop' value  } <$ _)
instance Attr Path_ RequiredFeatures (Event.Event Unit -> Event.Event  String ) where
  attr RequiredFeatures eventValue = unsafeAttribute (map (map ( \value -> { key: "requiredFeatures", value: prop' value })) eventValue)
...
instance Attr Pattern_ RequiredFeatures  String  where
  attr RequiredFeatures value = unsafeAttribute (  
    { key: "requiredFeatures", value: prop' value  } <$ _)
instance Attr Pattern_ RequiredFeatures (Event.Event Unit -> Event.Event  String ) where
  attr RequiredFeatures eventValue = unsafeAttribute (map (map ( \value -> { key: "requiredFeatures", value: prop' value })) eventValue)
...
instance Attr Polygon_ RequiredFeatures  String  where
  attr RequiredFeatures value = unsafeAttribute (  
    { key: "requiredFeatures", value: prop' value  } <$ _)
instance Attr Polygon_ RequiredFeatures (Event.Event Unit -> Event.Event  String ) where
  attr RequiredFeatures eventValue = unsafeAttribute (map (map ( \value -> { key: "requiredFeatures", value: prop' value })) eventValue)
...
instance Attr Polyline_ RequiredFeatures  String  where
  attr RequiredFeatures value = unsafeAttribute (  
    { key: "requiredFeatures", value: prop' value  } <$ _)
instance Attr Polyline_ RequiredFeatures (Event.Event Unit -> Event.Event  String ) where
  attr RequiredFeatures eventValue = unsafeAttribute (map (map ( \value -> { key: "requiredFeatures", value: prop' value })) eventValue)
...
instance Attr Rect_ RequiredFeatures  String  where
  attr RequiredFeatures value = unsafeAttribute (  
    { key: "requiredFeatures", value: prop' value  } <$ _)
instance Attr Rect_ RequiredFeatures (Event.Event Unit -> Event.Event  String ) where
  attr RequiredFeatures eventValue = unsafeAttribute (map (map ( \value -> { key: "requiredFeatures", value: prop' value })) eventValue)
...
instance Attr Svg_ RequiredFeatures  String  where
  attr RequiredFeatures value = unsafeAttribute (  
    { key: "requiredFeatures", value: prop' value  } <$ _)
instance Attr Svg_ RequiredFeatures (Event.Event Unit -> Event.Event  String ) where
  attr RequiredFeatures eventValue = unsafeAttribute (map (map ( \value -> { key: "requiredFeatures", value: prop' value })) eventValue)
...
instance Attr Switch_ RequiredFeatures  String  where
  attr RequiredFeatures value = unsafeAttribute (  
    { key: "requiredFeatures", value: prop' value  } <$ _)
instance Attr Switch_ RequiredFeatures (Event.Event Unit -> Event.Event  String ) where
  attr RequiredFeatures eventValue = unsafeAttribute (map (map ( \value -> { key: "requiredFeatures", value: prop' value })) eventValue)
...
instance Attr Text_ RequiredFeatures  String  where
  attr RequiredFeatures value = unsafeAttribute (  
    { key: "requiredFeatures", value: prop' value  } <$ _)
instance Attr Text_ RequiredFeatures (Event.Event Unit -> Event.Event  String ) where
  attr RequiredFeatures eventValue = unsafeAttribute (map (map ( \value -> { key: "requiredFeatures", value: prop' value })) eventValue)
...
instance Attr TextPath_ RequiredFeatures  String  where
  attr RequiredFeatures value = unsafeAttribute (  
    { key: "requiredFeatures", value: prop' value  } <$ _)
instance Attr TextPath_ RequiredFeatures (Event.Event Unit -> Event.Event  String ) where
  attr RequiredFeatures eventValue = unsafeAttribute (map (map ( \value -> { key: "requiredFeatures", value: prop' value })) eventValue)
...
instance Attr Tspan_ RequiredFeatures  String  where
  attr RequiredFeatures value = unsafeAttribute (  
    { key: "requiredFeatures", value: prop' value  } <$ _)
instance Attr Tspan_ RequiredFeatures (Event.Event Unit -> Event.Event  String ) where
  attr RequiredFeatures eventValue = unsafeAttribute (map (map ( \value -> { key: "requiredFeatures", value: prop' value })) eventValue)
...
instance Attr Use_ RequiredFeatures  String  where
  attr RequiredFeatures value = unsafeAttribute (  
    { key: "requiredFeatures", value: prop' value  } <$ _)
instance Attr Use_ RequiredFeatures (Event.Event Unit -> Event.Event  String ) where
  attr RequiredFeatures eventValue = unsafeAttribute (map (map ( \value -> { key: "requiredFeatures", value: prop' value })) eventValue)
...
instance Attr everything RequiredFeatures (Event.Event Unit -> Event.Event  Unit ) where
  attr RequiredFeatures eventValue = unsafeAttribute (map (map ( \_ -> { key: "requiredFeatures", value: unset' })) eventValue)
