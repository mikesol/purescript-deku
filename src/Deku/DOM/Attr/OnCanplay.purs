...
instance Attr anything OnCanplay (Event.Event Unit -> Event.Event  Cb ) where
  attr OnCanplay eventValue = unsafeAttribute (map (map (
    \value -> { key: "canplay", value: cb' value })) eventValue)
...
instance Attr anything OnCanplay (Event.Event Unit -> Event.Event  (Effect Unit) ) where
  attr OnCanplay eventValue = unsafeAttribute (map (map (
    \value -> { key: "canplay", value: cb' (Cb (const (value $> true))) })) eventValue)
...
instance Attr anything OnCanplay (Event.Event Unit -> Event.Event  (Effect Boolean) ) where
  attr OnCanplay eventValue = unsafeAttribute (map (map (
    \value -> { key: "canplay", value: cb' (Cb (const value)) })) eventValue) 
...
instance Attr everything OnCanplay (Event.Event Unit -> Event.Event  Unit ) where
  attr OnCanplay eventValue = unsafeAttribute (map (map (
    \_ -> { key: "canplay", value: unset' })) eventValue)
