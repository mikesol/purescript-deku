...
instance Attr Input_ Step (Event.Event Unit -> Event.Event  String ) where
  attr Step eventValue = unsafeAttribute (map (map ( \value ->
    { key: "step", value: prop' value })) eventValue)
...
instance Attr everything Step (Event.Event Unit -> Event.Event  Unit ) where
  attr Step eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "step", value: unset' })) eventValue)
