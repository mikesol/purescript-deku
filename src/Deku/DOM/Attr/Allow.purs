...
instance Attr Iframe_ Allow (Event.Event Unit -> Event.Event  String ) where
  attr Allow eventValue = unsafeAttribute (map (map (
    \value -> { key: "allow", value: prop' value })) eventValue)
...
instance Attr everything Allow (Event.Event Unit -> Event.Event  Unit ) where
  attr Allow eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "allow", value: unset' })) eventValue)
