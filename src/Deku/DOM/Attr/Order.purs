...
instance Attr FeConvolveMatrix_ Order (Event.Event Unit -> Event.Event  String ) where
  attr Order eventValue = unsafeAttribute (map (map (
    \value -> { key: "order", value: prop' value })) eventValue)
...
instance Attr everything Order (Event.Event Unit -> Event.Event  Unit ) where
  attr Order eventValue = unsafeAttribute (map (map ( \_ ->
    { key: "order", value: unset' })) eventValue)
