...
instance Attr anything OnMouseover (Effect Unit) where
  attr OnMouseover value = unsafeAttribute { key: "mouseover", value: cb' (Cb (const (value $> true))) }
instance Attr anything OnMouseover (Effect Boolean) where
  attr OnMouseover value = unsafeAttribute { key: "mouseover", value: cb' (Cb (const value)) }
instance Attr anything OnMouseover (Zora Unit) where
  attr OnMouseover value = unsafeAttribute { key: "mouseover", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnMouseover (Zora Boolean) where
  attr OnMouseover value = unsafeAttribute { key: "mouseover", value: cb' (Cb (const (runImpure value))) }