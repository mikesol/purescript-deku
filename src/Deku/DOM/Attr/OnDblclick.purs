...
instance Attr anything OnDblclick (Event.Event Unit -> Event.Event  Cb ) where
  attr OnDblclick eventValue = unsafeAttribute (map (map (
    \value -> { key: "dblclick", value: cb' value })) eventValue)
...
instance Attr anything OnDblclick (Event.Event Unit -> Event.Event  (Effect Unit) ) where
  attr OnDblclick eventValue = unsafeAttribute (map (map (
    \value -> { key: "dblclick", value: cb' (Cb (const (value $> true))) })) eventValue)
...
instance Attr anything OnDblclick (Event.Event Unit -> Event.Event  (Effect Boolean) ) where
  attr OnDblclick eventValue = unsafeAttribute (map (map (
    \value -> { key: "dblclick", value: cb' (Cb (const value)) })) eventValue) 
...
instance Attr everything OnDblclick (Event.Event Unit -> Event.Event  Unit ) where
  attr OnDblclick eventValue = unsafeAttribute (map (map (
    \_ -> { key: "dblclick", value: unset' })) eventValue)
