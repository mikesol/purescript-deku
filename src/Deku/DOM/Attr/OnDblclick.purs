...
instance Attr anything OnDblclick (Zora Unit) where
  attr OnDblclick value = unsafeAttribute { key: "dblclick", value: cb' (Cb (const (runImpure (value $> true)))) }
instance Attr anything OnDblclick (Zora Boolean) where
  attr OnDblclick value = unsafeAttribute { key: "dblclick", value: cb' (Cb (const (runImpure value))) }