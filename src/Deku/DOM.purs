...
-- codegen 2
import Deku.DOM.Elt.A(A_, class A_Ctor, a, a_)
import Deku.DOM.Elt.Abbr(Abbr_, class Abbr_Ctor, abbr, abbr_)
import Deku.DOM.Elt.Acronym(Acronym_, class Acronym_Ctor, acronym, acronym_)
import Deku.DOM.Elt.Address(Address_, class Address_Ctor, address, address_)
import Deku.DOM.Elt.Applet(Applet_, class Applet_Ctor, applet, applet_)
import Deku.DOM.Elt.Area(Area_, class Area_Ctor, area, area_)
import Deku.DOM.Elt.Article(Article_, class Article_Ctor, article, article_)
import Deku.DOM.Elt.Aside(Aside_, class Aside_Ctor, aside, aside_)
import Deku.DOM.Elt.Audio(Audio_, class Audio_Ctor, audio, audio_)
import Deku.DOM.Elt.B(B_, class B_Ctor, b, b_)
import Deku.DOM.Elt.Base(Base_, class Base_Ctor, base, base_)
import Deku.DOM.Elt.Basefont(Basefont_, class Basefont_Ctor, basefont, basefont_)
import Deku.DOM.Elt.Bdi(Bdi_, class Bdi_Ctor, bdi, bdi_)
import Deku.DOM.Elt.Bdo(Bdo_, class Bdo_Ctor, bdo, bdo_)
import Deku.DOM.Elt.Big(Big_, class Big_Ctor, big, big_)
import Deku.DOM.Elt.Blockquote(Blockquote_, class Blockquote_Ctor, blockquote, blockquote_)
import Deku.DOM.Elt.Body(Body_, class Body_Ctor, body, body_)
import Deku.DOM.Elt.Br(Br_, class Br_Ctor, br, br_)
import Deku.DOM.Elt.Button(Button_, class Button_Ctor, button, button_)
import Deku.DOM.Elt.Canvas(Canvas_, class Canvas_Ctor, canvas, canvas_)
import Deku.DOM.Elt.Caption(Caption_, class Caption_Ctor, caption, caption_)
import Deku.DOM.Elt.Center(Center_, class Center_Ctor, center, center_)
import Deku.DOM.Elt.Cite(Cite_, class Cite_Ctor, cite, cite_)
import Deku.DOM.Elt.Code(Code_, class Code_Ctor, code, code_)
import Deku.DOM.Elt.Col(Col_, class Col_Ctor, col, col_)
import Deku.DOM.Elt.Colgroup(Colgroup_, class Colgroup_Ctor, colgroup, colgroup_)
import Deku.DOM.Elt.Xdata(Xdata_, class Xdata_Ctor, xdata, xdata_)
import Deku.DOM.Elt.Datalist(Datalist_, class Datalist_Ctor, datalist, datalist_)
import Deku.DOM.Elt.Dd(Dd_, class Dd_Ctor, dd, dd_)
import Deku.DOM.Elt.Del(Del_, class Del_Ctor, del, del_)
import Deku.DOM.Elt.Details(Details_, class Details_Ctor, details, details_)
import Deku.DOM.Elt.Dfn(Dfn_, class Dfn_Ctor, dfn, dfn_)
import Deku.DOM.Elt.Dialog(Dialog_, class Dialog_Ctor, dialog, dialog_)
import Deku.DOM.Elt.Dir(Dir_, class Dir_Ctor, dir, dir_)
import Deku.DOM.Elt.Div(Div_, class Div_Ctor, div, div_)
import Deku.DOM.Elt.Dl(Dl_, class Dl_Ctor, dl, dl_)
import Deku.DOM.Elt.Dt(Dt_, class Dt_Ctor, dt, dt_)
import Deku.DOM.Elt.Em(Em_, class Em_Ctor, em, em_)
import Deku.DOM.Elt.Embed(Embed_, class Embed_Ctor, embed, embed_)
import Deku.DOM.Elt.Fieldset(Fieldset_, class Fieldset_Ctor, fieldset, fieldset_)
import Deku.DOM.Elt.Figcaption(Figcaption_, class Figcaption_Ctor, figcaption, figcaption_)
import Deku.DOM.Elt.Figure(Figure_, class Figure_Ctor, figure, figure_)
import Deku.DOM.Elt.Font(Font_, class Font_Ctor, font, font_)
import Deku.DOM.Elt.Footer(Footer_, class Footer_Ctor, footer, footer_)
import Deku.DOM.Elt.Form(Form_, class Form_Ctor, form, form_)
import Deku.DOM.Elt.Frame(Frame_, class Frame_Ctor, frame, frame_)
import Deku.DOM.Elt.Frameset(Frameset_, class Frameset_Ctor, frameset, frameset_)
import Deku.DOM.Elt.H1(H1_, class H1_Ctor, h1, h1_)
import Deku.DOM.Elt.H2(H2_, class H2_Ctor, h2, h2_)
import Deku.DOM.Elt.H3(H3_, class H3_Ctor, h3, h3_)
import Deku.DOM.Elt.H4(H4_, class H4_Ctor, h4, h4_)
import Deku.DOM.Elt.H5(H5_, class H5_Ctor, h5, h5_)
import Deku.DOM.Elt.H6(H6_, class H6_Ctor, h6, h6_)
import Deku.DOM.Elt.Head(Head_, class Head_Ctor, head, head_)
import Deku.DOM.Elt.Header(Header_, class Header_Ctor, header, header_)
import Deku.DOM.Elt.Hr(Hr_, class Hr_Ctor, hr, hr_)
import Deku.DOM.Elt.Html(Html_, class Html_Ctor, html, html_)
import Deku.DOM.Elt.I(I_, class I_Ctor, i, i_)
import Deku.DOM.Elt.Iframe(Iframe_, class Iframe_Ctor, iframe, iframe_)
import Deku.DOM.Elt.Img(Img_, class Img_Ctor, img, img_)
import Deku.DOM.Elt.Input(Input_, class Input_Ctor, input, input_)
import Deku.DOM.Elt.Ins(Ins_, class Ins_Ctor, ins, ins_)
import Deku.DOM.Elt.Kbd(Kbd_, class Kbd_Ctor, kbd, kbd_)
import Deku.DOM.Elt.Label(Label_, class Label_Ctor, label, label_)
import Deku.DOM.Elt.Legend(Legend_, class Legend_Ctor, legend, legend_)
import Deku.DOM.Elt.Li(Li_, class Li_Ctor, li, li_)
import Deku.DOM.Elt.Link(Link_, class Link_Ctor, link, link_)
import Deku.DOM.Elt.Main(Main_, class Main_Ctor, main, main_)
import Deku.DOM.Elt.Map(Map_, class Map_Ctor, map, map_)
import Deku.DOM.Elt.Mark(Mark_, class Mark_Ctor, mark, mark_)
import Deku.DOM.Elt.Meta(Meta_, class Meta_Ctor, meta, meta_)
import Deku.DOM.Elt.Meter(Meter_, class Meter_Ctor, meter, meter_)
import Deku.DOM.Elt.Nav(Nav_, class Nav_Ctor, nav, nav_)
import Deku.DOM.Elt.Noframes(Noframes_, class Noframes_Ctor, noframes, noframes_)
import Deku.DOM.Elt.Noscript(Noscript_, class Noscript_Ctor, noscript, noscript_)
import Deku.DOM.Elt.Object(Object_, class Object_Ctor, object, object_)
import Deku.DOM.Elt.Ol(Ol_, class Ol_Ctor, ol, ol_)
import Deku.DOM.Elt.Optgroup(Optgroup_, class Optgroup_Ctor, optgroup, optgroup_)
import Deku.DOM.Elt.Option(Option_, class Option_Ctor, option, option_)
import Deku.DOM.Elt.Output(Output_, class Output_Ctor, output, output_)
import Deku.DOM.Elt.P(P_, class P_Ctor, p, p_)
import Deku.DOM.Elt.Param(Param_, class Param_Ctor, param, param_)
import Deku.DOM.Elt.Picture(Picture_, class Picture_Ctor, picture, picture_)
import Deku.DOM.Elt.Pre(Pre_, class Pre_Ctor, pre, pre_)
import Deku.DOM.Elt.Progress(Progress_, class Progress_Ctor, progress, progress_)
import Deku.DOM.Elt.Q(Q_, class Q_Ctor, q, q_)
import Deku.DOM.Elt.Rp(Rp_, class Rp_Ctor, rp, rp_)
import Deku.DOM.Elt.Rt(Rt_, class Rt_Ctor, rt, rt_)
import Deku.DOM.Elt.Ruby(Ruby_, class Ruby_Ctor, ruby, ruby_)
import Deku.DOM.Elt.S(S_, class S_Ctor, s, s_)
import Deku.DOM.Elt.Samp(Samp_, class Samp_Ctor, samp, samp_)
import Deku.DOM.Elt.Script(Script_, class Script_Ctor, script, script_)
import Deku.DOM.Elt.Section(Section_, class Section_Ctor, section, section_)
import Deku.DOM.Elt.Select(Select_, class Select_Ctor, select, select_)
import Deku.DOM.Elt.Small(Small_, class Small_Ctor, small, small_)
import Deku.DOM.Elt.Source(Source_, class Source_Ctor, source, source_)
import Deku.DOM.Elt.Span(Span_, class Span_Ctor, span, span_)
import Deku.DOM.Elt.Strike(Strike_, class Strike_Ctor, strike, strike_)
import Deku.DOM.Elt.Strong(Strong_, class Strong_Ctor, strong, strong_)
import Deku.DOM.Elt.Style(Style_, class Style_Ctor, style, style_)
import Deku.DOM.Elt.Sub(Sub_, class Sub_Ctor, sub, sub_)
import Deku.DOM.Elt.Summary(Summary_, class Summary_Ctor, summary, summary_)
import Deku.DOM.Elt.Sup(Sup_, class Sup_Ctor, sup, sup_)
import Deku.DOM.Elt.Svg(Svg_, class Svg_Ctor, svg, svg_)
import Deku.DOM.Elt.Table(Table_, class Table_Ctor, table, table_)
import Deku.DOM.Elt.Tbody(Tbody_, class Tbody_Ctor, tbody, tbody_)
import Deku.DOM.Elt.Td(Td_, class Td_Ctor, td, td_)
import Deku.DOM.Elt.Template(Template_, class Template_Ctor, template, template_)
import Deku.DOM.Elt.Textarea(Textarea_, class Textarea_Ctor, textarea, textarea_)
import Deku.DOM.Elt.Tfoot(Tfoot_, class Tfoot_Ctor, tfoot, tfoot_)
import Deku.DOM.Elt.Th(Th_, class Th_Ctor, th, th_)
import Deku.DOM.Elt.Thead(Thead_, class Thead_Ctor, thead, thead_)
import Deku.DOM.Elt.Time(Time_, class Time_Ctor, time, time_)
import Deku.DOM.Elt.Title(Title_, class Title_Ctor, title, title_)
import Deku.DOM.Elt.Tr(Tr_, class Tr_Ctor, tr, tr_)
import Deku.DOM.Elt.Track(Track_, class Track_Ctor, track, track_)
import Deku.DOM.Elt.Tt(Tt_, class Tt_Ctor, tt, tt_)
import Deku.DOM.Elt.U(U_, class U_Ctor, u, u_)
import Deku.DOM.Elt.Ul(Ul_, class Ul_Ctor, ul, ul_)
import Deku.DOM.Elt.Var(Var_, class Var_Ctor, var, var_)
import Deku.DOM.Elt.Video(Video_, class Video_Ctor, video, video_)
import Deku.DOM.Elt.Wbr(Wbr_, class Wbr_Ctor, wbr, wbr_)
-- codegen 2
-- codegen 9
...
-- hack for canvas
data Draw2D = Draw2D
instance Attr Canvas_ Draw2D (Context2D -> Effect Unit) where
  attr Draw2D value = unsafeAttribute { key: "@canvas-hack@", value: cb' (Cb (unsafeCoerce value)) }