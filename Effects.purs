  ...
  ( \_ _ -> u
      { head: D.div []
          ...
          , pars: D.div []
              ( D.p []
                  ( D.text
                      """This example is similar to the previous one in its design: the looping function is called in response to an event. The difference is that the response isn't immediate. Instead, we wait for the result of a network call. Also, instead of using the rocket-ship syntax, we'll use the """
                        @@ D.code [] (S.text "io")
                        /\ D.text
                          """ syntax, which does the same thing but perhaps looks nicer to the eyes."""
                          /\ unit
                  )
                  @@
                    ( D.pre []
