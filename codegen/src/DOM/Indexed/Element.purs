...
imports
  :: Partial => String -> Foreign.Object Interface -> Array (ImportDecl Void)
imports baseMod interfaces =
  Array.concat
    ...
    , indexImports
        (bind (Foreign.values interfaces) \{ members } -> map snd members)
    , if Set.isEmpty missing then []
      else pure
        $ declImport baseMod
        $ map importType
        $ Array.fromFoldable missing
    ]
...
exports
  :: Partial => Foreign.Object Interface -> Array Element -> Array (Export Void)
...
generate interfaces tags = do
  Array.concat
    ...
    , bind (Foreign.values interfaces) \{ ctor, name, bases, members } ->
        [ declType ctor
            [ typeVarKinded "r" $ typeApp (typeCtor "Row") [ typeCtor "Type" ] ]
            $ typeIndex (coerce bases)
            -- add an additional property to identify the interface
            $ Array.cons
                (nominal /\ (typeApp (typeCtor "Proxy") [ typeString name ]))
    ...
    , bind tags \{ ctor: Ctor ctor, tag, ns, interface } -> do
        ...
        , declSignature shortShortHand
            $ typeArrow
                [ typeCtor "String" ]
                typeNut
        , declValue shortShortHand [ binderVar "t" ] $ exprApp (exprIdent ctor)
            [ exprArray []
            , exprArray
                [ exprApp (exprIdent "DC.text_")
                    [ exprIdent "t"
                    ]
                ]
            ]
...
typeIndex bases members =
  typeRow members $ Just $ Array.foldl (\r n -> typeApp (typeCtor n) [ r ])
    (typeVar "r")
    bases
...
crawlInterfaces elements all =
  ...
  reachable =
    go
      ( Foreign.fromFoldable $ map (\{ name, bases } -> name /\ coerce bases)
          all
      )
      initial
      initial
  ...
  go spec seen next = do
    let
      ...
      found =
        Set.fromFoldable
          $ Array.foldMap
              (fromMaybe mempty <<< flip Foreign.lookup spec <<< coerce)
