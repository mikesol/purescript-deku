...
generateSpec
  ...
  imports
  exports
  { elements, interfaces: all, attributes, events } = do
  let
    listenerMod :: String
    listenerMod =
      baseMod <> ".Listeners"
  ...
  FS.createDir $ Path.concat
    [ path
    , String.replaceAll (String.Pattern ".") (String.Replacement "/") baseMod
    ]
...
generate html svg mathml = do
  ...
  generateSpec "deku-dom/src" "Deku.DOM"
    [ unsafePartial $ declImportAs "Deku.Control"
        [ importValue "text", importValue "text_" ]
        "Deku.Control"
    ]
