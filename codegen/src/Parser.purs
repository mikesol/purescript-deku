...
generate = do
  classBaseText <- ExceptT $ attempt $ readTextFile UTF8
    "./codegen/src/Parser/Pursx.purs_"
  ...
  printError
    :: { error :: ParseError, position :: { line :: Int, column :: Int } }
    -> String
...
rewriteBody (ModuleBody body) = ModuleBody body
  { decls = expandClasses body.decls }

isClass :: ClassHead Void -> Type Void -> Boolean
isClass
  { name: Name { name: Proper l } }
  (TypeConstructor (QualifiedName { name: Proper r, module: Nothing })) =
  l == r
isClass _ _ =
  false
...
expandClasses decls = unsafePartial $ bind decls \decl -> case decl of
  ...
  DeclClass head _ | isClass head pxTagAttrValueWithWithDoubleQuoteClass ->
    Array.cons decl $ pxTagAttrValue head pxTagAttrValueWithWithDoubleQuoteClass
      punctuationWithSingleQuote

  DeclClass head _ | isClass head pxTagAttrValueWithWithSingleQuoteClass ->
    Array.cons decl $ pxTagAttrValue head pxTagAttrValueWithWithSingleQuoteClass
...
pxTagPreName { name } = Array.concat
  [ flip map whitespace \ws ->
      declInstance Nothing
        [ cons x y tail
        , typeApp pxTagPreNameClass
            [ verb, x, y, onezero, pursi, pathi, purso, patho, trailing ]
        ]
        name
...
pxTagName { name } = Array.concat
  [ flip map (lowerCase <> [ typeString "-" ] <> digits) \match ->
      declInstance Nothing
        ...
        , typeApp pxTagNameClass
            [ verb, tag, x, y, onezero, pursi, pathi, purso, patho, trailing ]
        ]
        name
        [ verb
        , tag_
...
preEndTagFromTrailing { name } = Array.concat
  ...
  , flip map (lowerCase <> [ typeString "-" ]) \match ->
      declInstance Nothing
        [ typeApp endTagFromTrailingClass
            [ match, tail, typeString "", tag, trailing ]
        ]
        name
      ...
      $ NEA.prependArray
          ...
          , declInstance Nothing
              ...
              , typeApp pxBodyClass
                  ...
                  , q
                  , r
                  , (rlcons (typeString "0") (typeString "0") onezero)
                  , pursi
                  , pathi
                  , purso
                  , patho
...
pxTagAttrName { name } = Array.concat
  ...
  , flip map whitespace \ws ->
      declInstance Nothing
        [ cons x y tail
        , typeApp pxTagPostAttrNameClass
            [ verb
            , hasAttributed
            , tag
            , x
...
pxTagPreAttrValue { name } =
  flip map whitespace \ws ->
    declInstance Nothing
      [ cons x y tail
      , typeApp pxTagPreAttrValueClass
          [ verb
          , hasAttributed
          , tag
          , x
...
pxTagAttrValue { name } kls punctuation =
  flip map
    ( lowerCase <> upperCase <> digits <> punctuation <> [ typeString "\\" ] <>
        whitespace
    )
    \match ->
      declInstance Nothing
