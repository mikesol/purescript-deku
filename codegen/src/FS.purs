...
cachedFetch
  :: forall @t. DecodeJson t => String -> String -> ExceptT Error Aff t
cachedFetch cacheName url = withExceptT
  (message >>> append (cacheName <> ": ") >>> error)
  do
...
createDir dirs = do
  let
    paths :: Array String
    paths = Array.scanl (\l r -> Path.concat [ l, r ]) "." $ String.split
      (String.Pattern "/")
      dirs
